...
The **_ThreadSync_** class allows to synchronize _Thread_ objects. It's algorythm is similar to OS Windows [Critical Section Objects](https://learn.microsoft.com/en-us/windows/win32/sync/critical-section-objects), except that the order of the queue is garantied to be sequential (FIFO). Every _ThreadSync_ object can serve up to 32 spinlocks or thread queues, using atomic bit operations. The difference between the two is that the first checks the bit and locks the thread without suspending it, while the second checks the bit and places the suspened thread into the queue until its turn. The first way is useful when setting a very few shared data variables via **`..Lock`** functions, where as the second one using using **`..Queue`** functions, is convenient when the waiting time could be significally longer. The **`isQueued`** function returns true, if the bit is currently involved in the thread queue synchronization. The _Thread_ class has a built-in static _ThreadSync_ object with direct access to its 30 of 32 bits, which is usually enough for a regular application needs.
