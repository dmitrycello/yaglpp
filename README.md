...
```
**_Rreference object_** is another kind of a _single id_ class object. It has the same functionality as automatic object, except that its destructor and **`delete..`** method only reset the class's data without clearing an OpenGL object _id_, leaving it to the source object. It could potentially contain invalid OpenGL _id_, and therefore should be used only as temporary asset in a current or another OpenGL context. In the long run, these objects should be tested via corresponding **`is..`** method. The reference object could be obtained as a result of several API calls, from the multi-object via **`get..`** method, or from another _single id_ class via **`refer..`** method. Using **`duplicate..`** method with reference object produce another reference object. The access to reference object does not have to be synchronized. The reference object's **`getObjectRef`** method always returns zero.
```
...
The multi-object could not be used by itself, every OpenGL _id_ can be used through a reference object obtained via one of its **`get..`** methods. The _ids_ whithin the same multi-object may be used with different _targets_. In Debug mode, it checks the object type at every assignment, since the usage of the same object _id_ with a different _target_ is not always safe (e.g. _Texture1D_ should not be later used as _Texture2D_):
```
