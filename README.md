...
The **_DataStore_** class controls the lifetime of allocated memory block. It is useful while working with the OpenGL [Buffer](https://www.khronos.org/opengl/wiki/Buffer_Object) object classes, as well as with _UniformBlock_ class. The **`createDataStore`** and **`deleteDataStore`** methods create and destroy the memory block. The **`getData`** and **`getSubData`** methods are used to get the pointer to a whole or a part of memory block. The **`loadData`** overloaded member functions allocate the memory and load data from a binary resource of **`RCDATA`** type, from a file, or from another DataStore object. At the same time, **`loadSubData`** loads a part of a source DataStore object. The appropriate constructors exist as well. The **`writeData`** and  **`writeSubData`** methods can save the memory block into a file. The **`copySubData`** replaces a part of destination memory block with a part of source memory block.
