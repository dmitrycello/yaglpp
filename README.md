...
YAGL++ is _yet another_ attempt to develop a C++ gear for the OpenGL API, merging its assets into the C++ objects. There are plenty of similar projects on the GitHub. So, why another one? The present library aims to fill the gap as a _thoughtful_, _transparent_ and _complete_ library, which includes every single API asset documented by [Khronoss](https://registry.khronos.org/OpenGL-Refpages/gl4/) reference pages. Primarily, YAGL++ was designed as a resource to help [Learn OpenGL](https://learnopengl.com/), but after a while it turned out to be quite efficient. It takes care of the routine work, allowing to develop a code in less complicated manner, and without substantial overhead. The C++ features make the OpenGL programming more intuitive, clear and stable. For example, using the [overloaded functions](https://learn.microsoft.com/en-us/cpp/cpp/function-overloading) is much more convenient than to recall the every exact API name, such as **`glUniform3fv`**. Instead, it is possible to just call **`Uniform::Set`** class method with the appropriate parameters.

The library is fully interchangeable with the original API assets, it consists only of the _header files_, and requires the C++ 11 compiler or later, so the basic knowledge of [C++ programming language](https://learn.microsoft.com/en-us/cpp/cpp/cpp-language-reference?view=msvc-170) is required to reading through the following section. The library uses standard library functions in its code, thus being cross-platform. However, it was initially designed to be used with MS Visual Studio under OS Windows. Therefore, the following features may be unavailable under different environment:

- Implementation of the [SAL](https://learn.microsoft.com/en-us/cpp/code-quality/using-sal-annotations-to-reduce-c-cpp-code-defects) concept (Microsoft Source Code Annotation Language). It makes the usage of the library less error-prone. The SAL macros are silently ignored in non-MSVC environment;
...
```
The library works with OpenGL context version 2.0 to 3.3, the higher versions are yet to come, and [GLFW](https://www.glfw.org/) version 3.4+. It is also integated with the [STB](https://github.com/nothings/stb) image library as one of its direct dependencies. The [Assimp](https://github.com/assimp/assimp) and [GLM](https://github.com/g-truc/glm) libraries are both written in C++, so they can be included as is, without any workaround. It is possible to select the required OpenGL context version by altering the **`YAGLPP_CONTEXT_VERSION_MAJOR`** and **`YAGLPP_CONTEXT_VERSION_MINOR`** main switches (default 3.3) in the [glpp.h](include/yaglpp/glpp.h) header file. The used context version affects the build, making available only the supported API assets. Every call to the API function in the library is provided with the appropriate error checking in Debug build. On the contrary, under the Release build, the library attempts to impliment the inline calls minimizing the overhead. Here is the average YAGL++ member function implementation under the Debug build:
```
...
```
### Naming concept
The YAGL++ library assets are defined within **`gl::`** and **`glfw::`** namespaces for GLAD and GLFW APIs respectively. The two helper classes, **`DataStore`** and **`StbImage`**, reside in the global namespace. You may bypass the **`gl::`** and **`glfw::`** prefixes with **`using namespace`** directives, but it is advised to keep them while managing large projects. For the sake of _transparency_, the original API names are mostly preserved. The names in YAGL++ library are obtained by stripping _gl_, _glfw_ or _stbi_ prefixes of the original API names, and by applying the Pascal-case rule for the rest, e.g., **`glDrawRangeElements`** in YAGL++ becomes **`gl::DrawRangeElements`**. The names in snake case are used in private assignments, and names starting with **`m_`** followed by Pascal-case stand for a private class member data. All symbols starting with **`YAGLPP_`** or **`yaglpp_`** are for library's internal use only.

Many of the class names are based on the OpenGL _targets_, e.g., **`Texture2D`** class name is based on **`GL_TEXTURE_2D`** target. But some changes could be applied in rare conflicting cases. For example, the **`GL_TEXTURE_BUFFER`** target is being used by both buffer and texture objects, therefore the **`TextureBuffer`** class is a _buffer_, while the **`BufferTexture`** is a _texture_. Nonetheless, the class type will always use the same target. As the API functions are grouped around the classes, the API constants are grouped around the _enum classes_. This approach makes the library less error-prone, since the usage of a wrong enumerator type or invalid value simply won't compile. The class member names are stripped of their object names since they are redundant, e.g., the original API function **`glBindTexture`** used with **`GL_TEXTURE_2D`** target in YAGL++ becomes **`gl::Texture2D::Bind`** class method. Some of the API functions such as **`glDisable`**, **`glEnable`**, **`glGet`**, **`glIsEnabled`**, and **`glPixelStore`** are used with their constant names. For example:
```
...
```
### IntelliSense
Every class, function or enum member in the library is provided with a comment shown in Visual Studio by the [IntelliSense](https://learn.microsoft.com/en-us/visualstudio/ide/using-intellisense) with the description, parameter list and return value. This is a nice opportunity to briefly recap what the function does while typing the code, instead of recoursing to the manual time after time. All information in such comments is taken from [Khronos](https://registry.khronos.org/OpenGL-Refpages/gl4/) website. If the description starts with the number in parentheses, it means either the minimal supported OpenGL context version (as floating point number), or the number of overloaded function (as integer number). The missing context version number assumes OpenGL 2.0 support. For example, the description of a function starting with **`(3.0) (3)`** means the support of OpenGL version 3.0 and above, plus it specifies that this is the third overloaded function. The description of the GLFW object function may start with **`(M)`** or **`(S)`** symbols meaning that the function can be called only from the _main_ or _secondary_ threads.

> [!IMPORTANT]
> The provided comments are brief, and if the function looks unfamiliar, it is recommended to consult its official API documentation.

### The main switches
The symbols defined after **`#ifndef YAGLPP_CONFIG`** statement at the beginning of [glpp.h](include/yaglpp/glpp.h) file are the _main switches_, they affect the library setup. All switches except the last two may be commented to unset the flag. The last **`YAGLPP_CONTEXT_VERSION_MAJOR`** and **`YAGLPP_CONTEXT_VERSION_MINOR`** switches must be set to a value:
...
- Commenting the **`YAGLPP_CLASS_PROPERTIES`** switch will exlude the class properties from the library, they are unavailable while being in non-MSVC environment regardless of the flag;
...
- The **`YAGLPP_STRICT_CONTEXT`** switch forces the YAGL++ to load the OpenGL verion set by the **`YAGLPP_CONTEXT_VERSION_MAJOR`** and **`YAGLPP_CONTEXT_VERSION_MINOR`** switches with _Core Profile_, but if the version is 3.1 or below, it will keep the default _Compate Profile_. Commenting this switch will load the highest available version with the default profile, ensuring that the loading will never fail. It is possible to eximine the loaded version by using **`IsContextVersion(major,minor)`** macro, or by appropriate window attributes. The loading algorithm could be overwritten by using  **`glfw::ContextSetting`** stucture while creating window object;
- Finally, the **`YAGLPP_CONTEXT_VERSION_MAJOR`** and **`YAGLPP_CONTEXT_VERSION_MINOR`** switches specify the OpenGL context version the library provides. They also limit the scope of the library, making available only the supported assets. Accepted combination of these values are: 2/0, 2/1, 3/0, 3/1, 3/2 and 3/3 to represent the supported versions 2.0 to 3.3 respectively;

> [!TIP]
> It is possible to override the main swithes, placing them inside a project. To do that, first define a symbol **`YAGLPP_CONFIG`**, then all the above switches, prior to include the main [glpp.h](include/yaglpp/glpp.h) header file.

### GLAD classes
The classes within the **`gl::`** namespace are counterparts of the GLAD API. Every GLAD class have a copy constructor, allowing to use it in an assignment statement, as a function parameter, or as a return value. Every class has a unique data member: either the unsigned 4-byte integer in _object_ classes, or the signed 4-byte integer in _location_ classes. The _object_ class contains the OpenGL object id, while the _location_ class carries the index value retrieved from the linked program object. Both classes have no destructor, since the single data value does not require cleanup. The **`Sync`** class object contains single pointer, and its OpenGL object must be explicitly deleted at some point in the code, so it has no destructor as well. The **`UniformBlock`** class is considered as _location_ class, but it does have a destructor, since several its data members require cleanup. The GLAD class object by design cares only about its lifetime. If it goes out of scope, it only cleans up itself, without deleting its real OpenGL object, which can still be present, and retrieved via various functions. This policy allows to prevent the cases when C++ object unintentionally deletes an OpenGL object while being assigned, copied or ceasing to exist. However, when necessary, every _object_ class or **`Sync`** class can be instantiated via **`Auto`** template, deleting its OpenGL object when it goes out of scope:
```
gl::Auto<gl::Texture2D> texture;
    // cleaned up when out of scope
```
The _object_ classes are derived from **`Objects`**, their default constructor creates an empty object, allowing the class to exist before OpenGL initialization. The object is considered empty, if its data member is set to zero, in this case its **`IsObject`** member function returns false. All derived _object_ classes have the same data size as their parent classes, thus allowing to easily combine different types of objects within a stucture or another class. They use various _templates_ making the library more compact, so the IntelliSense may often show the class name as **`ClassName_1`**, **`ClassName_2`** etc. referring to the same class. The _location_ classes have no base class, their default constructor also creates an empty object, which is when the location equals to **`-1`**, or invalid location value. The _location_ value does not require a cleanup, since its lifetime rely on the program object. Some class constructors create, bind, and in some cases initialize their OpenGL objects. Here are some examples with the YAGL++ code and its API equivalent quasi-code written as comments:
```
gl::ArrayBuffer vbo(data, usage);
    // glGenBuffers(1, &vbo.id);
    // glBindBuffer(GL_ARRAY_BUFFER, vbo.id);
    // glBufferData(GL_ARRAY_BUFFER, data.GetSize(), data.Get(), usage);

gl::Sampler sampler = gl::Sampler::GetBinding();
    // glGetIntegerv(GL_SAMPLER_BINDING, &sampler.id);

GLboolean b1 = sampler.IsSampler();
    // b1 = glIsSampler(sampler.id);

GLboolean b2 = sampler.IsBinding();
    // glGetIntegerv(GL_SAMPLER_BINDING, &id);
    // b2 = (sampler.id == id);
...
```
### GLAD tree
The following figure shows the GLAD class hierarchy chart. The **`Objects`** is the base class for almost every GLAD class, it is the only one having the red frame, meaning that it is an abstract class and cannot be created. Every class derived from **`Objects`** on the right side of the figure, controls the OpenGL [Buffer](https://www.khronos.org/opengl/wiki/Buffer_Object), [Framebuffer](https://www.khronos.org/opengl/wiki/Framebuffer_Object), [Query](https://www.khronos.org/opengl/wiki/Query_Object), [Renderbuffer](https://www.khronos.org/opengl/wiki/Renderbuffer_Object), [Sampler](https://www.khronos.org/opengl/wiki/Sampler_Object), [Texture](https://www.khronos.org/opengl/wiki/Texture), and [Vertex Array](https://www.khronos.org/opengl/wiki/Vertex_Specification#Vertex_Array_Object) objects, using their particular API target, and therefore could be called a _target objects_. The [Program](https://www.khronos.org/opengl/wiki/GLSL_Object#Program_objects) and [Shader](https://www.khronos.org/opengl/wiki/Shader) objects use different mechanism: while the target objects should be _binded_ to its target in order to operate, the shader object must be _compiled_ before being attached to a program, where as the program object must be _linked_ after the compiled shader objects are attached.

The first five classes directly derived from **`Objects`**, are subtype classes, they can be directly created, but without much of the functionality. They can be used as the temporary assets before being casted to one of its child class, when the resulting type is unknown. However, the precaution should be taken, once OpenGL object is initialized as particular type, using it as another type may result in OpenGL error. The dynamic allocation using operators _new[]_ and _delete[]_ for all classes derived from **`Objects`** is not allowed. The array of _object_ classes should be generated with **`Alloc`** and deleted with **`Dealloc`** static member funtions. Another option is to use **`GenMap`** and **`DeleteMap`**, which sets up and clears the user-defined stucture of classes derived from the same subtype (e.g., **`ArrayBuffer`**, **`UniformBuffer`**, etc). Since the shader object cannot be created without knowing its type, its **`Alloc`** function returns an array of empty **`Shaders`** classes.

![00-readme-2](docs/00-readme-2e.png)

The ***Uniform*** and ***VertexAttrib*** classes have no destructors, they operate the location index value of a [Uniform](https://www.khronos.org/opengl/wiki/Uniform_(GLSL)) and a [Vertex Attribute](https://www.khronos.org/opengl/wiki/Vertex_Specification) respectively, which remains valid as long as their [Program](https://www.khronos.org/opengl/wiki/GLSL_Object#Program_objects) object remains linked. They have a default constructor creating an _empty_ object, as well as a copy constructor copying the location value. The **`GetLocation`** method aquires the location index using a program object and the uniform or attribute name. If the location index is known, it could be set directly with **`SetLocation`** method. The appropriate constructors exist as well. To find out if the location value was set successfully, **`IsUniform`** and **`IsVertexAttrib`** methods should be used.

The ***Sync*** class operates the OpenGL [Synchronization](https://www.khronos.org/opengl/wiki/Sync_Object) object, which is the pointer to an opaque API object. It must be created at the specific point of a program with **`Fence`** method, therefore its methods **`ClientWait`**, **`GetStatus`** and **`Wait`** require the valid object.

The ***UniformBlock*** class allows to operate many uniform variables at once, it works closely with [Uniform Buffer Object](https://www.khronos.org/opengl/wiki/Uniform_Buffer_Object). The class data has a uniform block index, which is aquired with **`GetIndex`** or directly set with **`SetIndex`** methods, or by the appropriate constructor. These methods retrieve as well the program _id_, number of uniforms, data block size and the array of uniform block indices. This allows to use the saved data in next operations without the need to save it outside of a class. The _offset map_ is a special class feature allowing much easier uniform block data exchange. It is basically a user-defined _stucture_ of pointers of particular types mirroring the _uniform block_ types specified in GLSL shader. The **`OffsetMap`** method calculates all uniform offsets in the specified memory block, and sets the absolute addresses within the uniform buffer data to the offset map's pointers, allowing to interchange uniform values directly through these pointers.

### GLFW classes
The classes in the **`glfw::`** namespace are counterparts of the GLFW API. They all have the default constructor creating an empty class object, but unlike the GLAD objects, GLFW objects do not require binding, they must be explicitly created or selected via their methods. Consisting of multiple data members, these cumbersome objects are less suitable to be combined within a stucture or another class, and therefore they are not supplied with a copy constructor or any duplicate function. According to the [GLFW documentation](https://www.glfw.org/docs/latest/intro_guide.html#thread_safety), the most of GLFW API functions _must only be called from the main thread_. Despite the GLFW library itself does not raise an error, the YAGL++ library will always assert the proper calling of these functions only in Debug mode, since otherwise their behaviour on the different platforms is not guaranteed. Starting from the version 3.4, YAGL++ use GLFW custom allocator, this allows customized memory managment via the general library macros.

The **`Cursor`** class operates a GLFW [Cursor](https://www.glfw.org/docs/3.3/input_guide.html#cursor_object) object. It has only **`createStandardCursor`** method and two overloaded **`createCursor`** methods to explicitly create the GLFW cursor object from different sources. The appropriate constructors exist as well. The **`destroyCursor`** method is called from the class destructor, if the object was not cleared before.
