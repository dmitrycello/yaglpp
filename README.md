...
```
The _id_ classes are derived from **`Objects`**, their default constructor creates an empty object. The object is considered empty, if its data member is set to zero, in this case its **`IsObject`** member function returns false. All derived _id_ classes have the same data size as their parent classes, thus allowing to easily combine different types of objects within a stucture or another class. They use various _templates_ making the library more compact, so the IntelliSense may often show the class name as **`ClassName_1`**, **`ClassName_2`** etc. meaning the same type. The _location_ classes have no base class, their default constructor also creates an empty object, which is when the location equals to **`-1`**, or invalid location value. The _location_ value does not require a cleanup, since its lifetime rely on the program object. Some class constructors create, bind, and sometimes even initialize their OpenGL objects. Here are some examples with the YAGL++ code and its API equivalent quasi-code written as comments:
```
