...
The multi-object could not be used by itself, every OpenGL _id_ can be used through a reference object obtained via one of its **`get..`** methods. The _ids_ whithin the same multi-object may be used with different _targets_. In Debug mode, it checks the object type at every assignment, if it is uses the same target, since the usage of the same object _id_ with a different _target_ is not always safe (e.g. _Texture1D_ should not be later used as _Texture2D_). The access to multi-object should be synchronized when duplicating or deleting an object, but it is safe to use it in any other operation, except it the first cell assignment while in Debug mode.
```
