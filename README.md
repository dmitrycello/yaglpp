...
**_Automatic object_** is a _single id_ class object creating its own OpenGL object either implicitly through "lasy" creation, or explicitly via **`gen..`** method. The access to an OpenGL _id_ is gained through a global internal stucture, which keeps track of all object's copies using reference counter. The class object could be duplicated by the copy constructor or via **`duplicate..`** method increasing object's reference counter, or cleared by the destructor or via **`delete..`** method decreasing the counter. When the last instance of the class is being cleared, it deletes the OpenGL object. The number of instances could be retreived via **`getObjectRef`** method, which always returns the positive value.
```
