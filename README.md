...
```
**_Rreference object_** is another kind of a _single id_ class object. It has the same functionality as automatic object, except that its destructor and **`delete..`** method only reset the class's data. It does not control the OpenGL object's lifetime, leaving it to the source object. It could potentially contain invalid OpenGL _id_, and therefore should be used be used only as temporary asset in a current or another OpenGL context. The reference object could be obtained as a result of several API calls, from the multi-object via **`get..`** method, or from another _single id_ class via **`refer..`** method. Using **`duplicate..`** method with reference object produce another reference object. The reference object's **`getObjectRef`** method always returns zero.
```
