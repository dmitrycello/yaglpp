...
The first five classes directly derived from **`gl::Objects`**, are subtype classes, they can be directly created, but without being much useful. They can be used as the temporary asset before being casted to one of its child class, when the resulting type is unknown. However, the precaution should be taken, once OpenGL object is initialized as particular type, using it as another type may result in OpenGL error. The dynamic allocation using operators _new_, _new[]_, _delete_ and _delete[]_ for all classes derived from **`gl::Objects`** is not allowed. Instead, an array of target classes could be generated with **`Alloc`** and cleared with **`Dealloc`** static member funtions. Another option is to use **`GenMap`** and **`DeleteMap`**, which sets up and clears the user-defined stucture of classes derived from the same subtype class (e.g., **`gl::ArrayBuffer`**, **`gl::UniformBuffer`**, etc).
