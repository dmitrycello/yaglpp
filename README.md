...
The **_Monitor_** class is somewhat similar to Joystick class, it controls GLFW [monitor object](https://www.glfw.org/docs/3.3/monitor_guide.html#monitor_object), which exist from the GLFW initialization. The monitor device could be _selected_ via appropriate constructor or three overloaded **`selectMonitor`** methods, and _deselected_ via class destructor or **`deselectMonitor`** method. As for Joystick class, it is not possible to have multiple Monitor classes referencing the same monitor device, so the same mechanism is applied here. And as Joystick class, Monitor class has **`onDisconnect`** virtual function. To catch monitor connected event, set **`MonitorConnectedCallback`** procdure via **`setMonitorkConnectedCallback`** global function or static method.

The **_Thread_** class is not a part of GLFW library, it is added to **`glfw::`** namespace exclusively to handle OS Windows application threads. Its default constructor sets up an empty thread object, except that the main application thread object exist since the GLFW initialization, and can be retreived through **`getMainThread`** global function or static method. If the YAGL++ library is set into an AFX-alike mode, this function returns a pointer to a global application variable. The class allows to set up inter-thread communication and synchronization mechanisms, as well as to handle thread specific parameters, such as thread stutus, priority etc. The Thread class supports event handling, the virtual functions **`onInitThread`**, **`onRunThread`** and **`onExitThread`** notify the class object at every corresponding stage. The **_ThreadWnd_** class derived from the Thread class, is designed to work with GLFW window. It has also **`onPostMessage`** virtual function, the thread post message event handler, and **`onRenderWindow`** virtual function, the event handler triggered at every iteration of GLFW window rendering loop.
