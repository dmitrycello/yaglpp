...
> [!NOTE]
> Some of YAGL++ objects do not follow this rule. The **`gl::VertexArray`** object must be _bound_ explicitly at a certain point of the code, the **`gl::Sampler`** must be _bound_ with a parameter, the **`gl::Sync`** object must be _created_ explicitly with **`fenceSync`** method, where as _multi-objects_ and all **`glfw::`** objects have to be _created_ with specific parameters.
