...
The first five classes directly derived from **`Objects`**, are subtype classes, they can be directly created, but without much of the functionality. They can be used as the temporary assets before being casted to one of its child class, when the resulting type is unknown. However, the precaution should be taken, once OpenGL object is initialized as particular type, using it as another type may result in OpenGL error. The dynamic allocation using operators _new_, _new[]_, _delete_ and _delete[]_ for all classes derived from **`Objects`** is not allowed. The array of _object_ classes should be generated with **`Alloc`** and deleted with **`Dealloc`** static member funtions. Another option is to use **`GenMap`** and **`DeleteMap`**, which sets up and clears the user-defined stucture of classes derived from the same subtype (e.g., **`ArrayBuffer`**, **`UniformBuffer`**, etc).
