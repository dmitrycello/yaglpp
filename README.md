...
### GLAD classes
The classes within the _gl::_ namespace are counterparts of the GLAD API. The GLAD class object only cares about its lifetime. If it goes out of scope, it only cleans up itself, without deleting its real OpenGL object, while the OpenGL object can still be retrieved via various functions. All classes have the default constructor creating an empty class object, this allows the class to exist before OpenGL initialization. Every GLAD class have a copy constructor, duplicating the source object, this allows to use it in an assignment statement, as a function parameter, or as a return value. Every class has a unique data member: either the unsigned 4-byte integer in _id_ classes, or the signed 4-byte integer in _location_ classes. The _id_ class contains the OpenGL object id, while the _location_ class carries the index value retrieved from the linked program object. Both classes have no destructor, since the single data value does not require a cleanup. The two exceptional classes are **`Sync`** and **`UniformBlock`**, the first contains single pointer, and the second has several data members.

All derived classes have the same data size as their parent classes, thus allowing to easily combine different types of objects within a stucture or another class. The _id_ classes are derived from **`gl::Objects`**, their default constructor creates an empty object. The object is considered empty, if its data member is set to zero, in this case its **`IsObject`** member function returns false. The _location_ classes have no base class, their default constructor also creates an empty object, which is when the location equals to **`-1`**, or invalid location value. Some class constructors create, bind, and sometimes even initialize their OpenGL objects. Here are some examples with the YAGL++ code and its API equivalent quasi-code written as comments:
```
...
```
### GLAD tree
The following figure shows the GLAD class hierarchy chart. The **`gl::Objects`** is the base class for almost every GLAD class, it is the only one having the red frame, meaning that it is an abstract class and cannot be created. Every class derived from **`gl::Objects`** on the right side of the figure, controls the OpenGL [Buffer](https://www.khronos.org/opengl/wiki/Buffer_Object), [Framebuffer](https://www.khronos.org/opengl/wiki/Framebuffer_Object), [Query](https://www.khronos.org/opengl/wiki/Query_Object), [Renderbuffer](https://www.khronos.org/opengl/wiki/Renderbuffer_Object), [Sampler](https://www.khronos.org/opengl/wiki/Sampler_Object), [Texture](https://www.khronos.org/opengl/wiki/Texture), and [Vertex Array](https://www.khronos.org/opengl/wiki/Vertex_Specification#Vertex_Array_Object) objects, using their particular API target, and therefore could be called a _target class_. The classes controling the OpenGL [Program](https://www.khronos.org/opengl/wiki/GLSL_Object#Program_objects) and [Shader](https://www.khronos.org/opengl/wiki/Shader) objects use different mechanism: the target class should be _binded_ to its target in order to operate, the shader class must be _compiled_ before being attached to a program, where as the program class must be _linked_ after the compiled shader classes are attached.

The first five classes directly derived from **`gl::Objects`**, are subtype classes, they can be directly created, but without being much useful. They can be used as the temporary asset before being casted to one of its child class, when the resulting type is unknown. However, the precaution should be taken, once OpenGL object is initialized as particular type, using it as another type may result in OpenGL error. The dynamic allocation using operators _new_, _new[]_, _delete_ and _delete[]_ for all classes derived from **`gl::Objects`** is not allowed. The only array of _target_ classes could be generated with **`Alloc`** and deleted with **`Dealloc`** static member funtions. Another option is to use **`GenMap`** and **`DeleteMap`**, which sets up and clears the user-defined stucture of classes derived from the same subtype (e.g., **`gl::ArrayBuffer`**, **`gl::UniformBuffer`**, etc). The program and shader classes must be created one by one.
