...
```
**_Rreference object_** is another kind of a _single id_ class object. It has the same functionality as automatic object, except that its destructor and **`delete..`** method only reset its data without clearing an OpenGL object _id_, leaving it to the source object. It could potentially contain invalid OpenGL _id_, and therefore should be used only as temporary asset in a current or another OpenGL context. In the long run, these objects could be tested via corresponding **`is..`** method. The reference object could be obtained as a result of several API calls, from the multi-object via **`get..`** method, or from another _single id_ class via **`refer..`** method. Using **`duplicate..`** method with reference object produce another reference object. The access to reference object does not have to be synchronized, so it is the best to be used in another context. The call to reference object's **`getObjectRef`** method always returns zero.
```
