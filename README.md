...
The **_ThreadWnd_** class derived from **`glfw::Thread`**, is designed to work with GLFW window. It has pointer to Window class data member, and two more virtual functions: **`onPostMessage`**, the thread post message event handler, and **`onRenderWindow`**, the event handler triggered at every iteration of GLFW window rendering loop.
