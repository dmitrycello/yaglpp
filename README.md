...
All derived classes have the same data size as their parent classes, thus allowing to easily combine them within another stucture or class. Every class object is considered _empty_, if its data member is set to zero, in this case its **`IsObject`** function returns false. The _id_ classes are derived from **`gl::_Objects`**, some of them have constructors creating, binding, and sometimes even initializing their OpenGL objects. Here are some examples of YAGL++ code vs its API implementation:
```
gl::ArrayBuffer vbo(data, usage);    // glGenBuffers(1, &vbo.id);
                                     // glBindBuffer(GL_ARRAY_BUFFER, vbo.id);
                                     // glBufferData(GL_ARRAY_BUFFER, data.GetSize(), data.Get(), usage);
sampler = gl::Sampler::GetBinding(); // glGetIntegerv(GL_SAMPLER_BINDING, &sampler.id);
GLboolean b1 = sampler.IsSampler();  // glIsSampler(sampler.id);
GLboolean b2 = sampler.IsBinding();  // glGetIntegerv(GL_SAMPLER_BINDING, &id);
                                     // sampler.id == id;
```
### Class tree
