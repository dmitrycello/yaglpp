...
> [!TIP]
> While the YAGL++ library is not in AFX-alike mode, the **`getMainThread`** function returns the pointer to **`glfw::Thread`** base class. Despite that its virtual functions could not be overwritten, it is still possible to handle its post messages within the rendering loop.
