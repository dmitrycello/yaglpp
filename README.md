...
The **_Thread_** class is not a part of GLFW library, it is added to **`glfw::`** namespace exclusively to handle the OS Windows [Threads](https://learn.microsoft.com/en-us/windows/win32/procthread/processes-and-threads). Its default constructor sets up an empty object, except that the main application Thread object exists since the GLFW initialization, it can be retreived through **`getMainThread`** global function or a static method. If the YAGL++ library is set into an AFX-alike mode, this function returns a pointer to a global application variable. Every other class object can start a new application thread. The Thread class allows to set up inter-thread communication, as well as to handle thread specific parameters, such as thread status, priority etc. Its virtual functions **`onInitThread`**, **`onRunThread`** and **`onExitThread`** notify the class object at every corresponding thread stage.

The **_ThreadSync_** class allows to synchronize Thread objects. It's algorythm is similar to OS Windows [Critical Section Objects](https://learn.microsoft.com/en-us/windows/win32/sync/critical-section-objects), except that the order of the queue is garantied to be sequential (FIFO). Every object can serve up to 32 spinlocks or thread queues, using atomic bit operations. The difference between the two is that the first checks the bit and locks the thread without suspending it, and the second checks the bit and places the suspened thread to the queue until its turn. The first way is useful when setting a very few shared data variables via **`enterLock`**, **`leaveLock`** and **`tryLock`** functions, where as the second way is convenient when the waiting time could be significally longer, using **`enterQueue`**, **`leaveQueue`** and **`tryQueue`** functions. The **`isQueued`** function returns true, if the bit is currently involved in the thread queue synchronization. The _Thread_ class has one built-in static _ThreadSync_ object with direct access to its 30 of 32 bits, which is usually enough for a regular application needs.

The **_WindowThread_** class derived from **`glfw::Thread`**, is designed to work with GLFW window. It has pointer to Window class data member, and two more virtual functions: **`onPostMessage`**, the thread post message event handler, and **`onRenderWindow`**, the event handler triggered at every iteration of GLFW window rendering loop, which includes the **`glfwWindowShouldClose`**, **`glfwSwapBuffers`**, and **`glfwPollEvents`** API calls.
