...
The **_Cursor_** class operates a GLWF [cursor object](https://www.glfw.org/docs/3.3/input_guide.html#cursor_object). It has only **`createStandardCursor`** method and five **`createCursor`** overloaded methods to explicitly create the GLFW cursor object from different sources. The appropriate constructors exist as well. The **`destroyCursor`** method is called from the class destructor, if the object was not cleared before.
