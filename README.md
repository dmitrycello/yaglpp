...
### Naming concept
For the sake of transparency, the original API names are carefully preserved. The most of the class names are based on the OpenGL _targets_, e.g. **`Texture2D`** class name is based on **`GL_TEXTURE_2D`** target. In case of some rare conflicting cases, e.g. when buffer and texture are using the same **`GL_TEXTURE_BUFFER`** target, the _BufferTexture_ is a texture, but the _TextureBuffer_ is a buffer. As the API functions are grouped around the classes, the API constants are grouped around the enum classes. This approach allows to clearly understand what the _object_ is, what the _function_ does, and what the _enum_ class member means. It also makes the library less error-prone, since the usage of a wrong object type or enumerator value won't compile.
