...
The installation of the YAGL++ library is basically a collection of the original OpenGL API components. The offered setup allows to build an application using the dynamic runtime libraries in _Debug_ mode, and the static runtime libraries in _Realease_ mode. While in Debug mode the application has lower portability, the dynamic linking takes less disk space, and works better with Visual Studio debugger. In the countrary, in Release mode the static linking makes the application file slightly bigger, but in return it improves the component loading speed along with the better compatibility, which is the preferred way in the game developper world. To be honest, the static runtime scheme is trickier, since many components are using DLL runtime libraries by default. It also takes significant space on the hard drive, but the the game is worth the candle. Anyways, it is always possible to return to dynamic runtime scheme, and thus save the space. The path layout can be different, but it is strongly recommended to use the suggested way, at least for the first time. The setup supports _x64_ and _Win32_ platforms under Microsoft Visual Studio 2019 or later, producing the _Console application_ in Debug mode, and _Windows application_ in Release mode.
...
- [Assimp](https://github.com/assimp/assimp/releases) source package v6.0.5 or later;
...
Create the new folder under the name **`Common`** within the newly created solution directory. In that folder create five (5) subfolders: **`bin`**, **`include`**, **`lib`**, **`res`**, and **`src`**. Create two subfolders **`Win32`** within **`Common/bin`** and **`Common/lib`** directories. The **`Win32`** subfolders contain the binaries for _Win32_ platform. The **`Common/src`** directory keeps the source files required for debugging through the component, and the **`Common/res`** directory contains the common resources of different kinds. To follow the [Learn OpenGL](https://learnopengl.com/) tutorial, download the resources from [here](https://github.com/JoeyDeVries/LearnOpenGL/tree/master/resources), and move them into this subfolder. At the moment they all could be left empty. The folder tree should be as following:

![02-common-dirs-1](02-common-dirs-1a.png)
