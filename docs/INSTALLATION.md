...
The installation of the YAGL++ library is basically a collection of the original OpenGL API components. The offered setup allows to build an application with the dynamic runtime libraries in _Debug_ mode, and with the static runtime libraries in _Realease_ mode. While in Debug mode the application has lower portability, the setup takes less disk space, and works faster, because system DLL are already in the computer memory. In the countrary, in Release mode the setup makes the application file slightly bigger, but in return it improves the component loading speed along with the better compatibility, which is the preferred way in the game developper world. To be honest, this way is trickier, since many components are using DLL runtime by default. It also takes significant space on the hard drive (around 500mb), but the the game is worth the candle. Anyways, it is always possible to revert to dynamic runtime scheme, and thus save the space. The path layout can be different, but it is strongly recommended to use the suggested way, at least for the first time. The setup supports _x64_ and _Win32_ platforms under Microsoft Visual Studio 2019 or later, producing the _Console application_ in Debug mode, and _Windows application_ in Release mode.
