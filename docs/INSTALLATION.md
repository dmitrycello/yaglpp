...
> [!NOTE]
> The GLFW library folder name depends on used IDE. Extract the appropriate folder from the binary archive into the required _lib_ subfolder, then move dll file to the required _bin_ subfolder. If different to Visual Studio 2019, make sure to adjust the **`GLPP_GLFW_LIB`** main switch in [glpp.h](include/glpp.h) file.
