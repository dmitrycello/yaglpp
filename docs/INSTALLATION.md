...
Create the new folder under the name **`Common`** within the newly created solution directory. In that folder create four (4) subfolders: **`bin`**, **`include`**, **`lib`**, and **`res`**. Create two subfolders **`Win32`** within **`Common/bin`** and **`Common/lib`** directories. The **`Common/bin`** and **`Common/bin/Win32`** directories contain the common binaries for _x64_ and _Win32_ projects. The **`Common/res`** directory contains the common resources of different kinds. To follow the [Learn OpenGL](https://learnopengl.com/) tutorial, download the resources from [here](https://github.com/JoeyDeVries/LearnOpenGL/tree/master/resources), and move them into this subfolder. At the moment they all could be left empty. The folder tree should be as following:
