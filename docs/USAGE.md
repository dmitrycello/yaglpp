...
At this point, it would be wise to save all performed work by creating a Visual Studio project template from the current project. Later, it would be possible to create a new project, without the need to set all required parameters. The template will work with the similar solution path layout. Download the [template icon](template_icon.png) and [template preview](template_preview.png) files, or use any other with transparent background. The source code could be minimized to the following:
```
// main.cpp
#include "config.h"
int main(int argc, char** argv)
