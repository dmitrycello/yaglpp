...
int main(int argc, char** argv)
{
	...
	program.useProgram();
	gl::Uniform mixer(program, "mixer"), transform(program, "transform");
	...
	while (!window.windowShouldClose())
	{
		...
		trans = glm::rotate(trans, (float)glfw::getTime(), glm::vec3(0.0f, 0.0f, 1.0f));
		transform.uniformMatrix4(1, GL_FALSE, glm::value_ptr(trans));
