...
#ifndef GLPP_NO_AFX_LAYOUT
int main(int argc, char** argv)
{
    return glfw::Thread::_main(argc, argv);
}
#endif // #ifndef GLPP_NO_AFX_LAYOUT
...
{
    GLPP_ASSERT(!isThread()); // GLFW THREAD OBJECT HAS CURRENTLY RUNNING THREAD
...
{
    bool bResult;
    GLPP_ASSERT(isCurrentThread()); // METHOD IS CALLED BY ANOTHER THREAD
...
{
    GLPP_ASSERT(!isCurrentThread()); // METHOD IS CALLED BY THE CURRENT THREAD
    _LPPACK lpPack = (_LPPACK)GLPP_MALLOC(sizeof(_PACK));
    lpPack->next = nullptr;
    lpPack->msgid = msgid;
...
{
    GLPP_ASSERT((DWORD)processor < MAXIMUM_PROCESSORS); // INVALID INDEX OF IDEAL PROCESSOR FOR A THREAD OBJECT
...
{
    GLPP_ASSERT(m_hThread != NULL); // GLFW THREAD OBJECT HAS NO RUNNING THREAD
    return m_hThread;
}

void Thread::enterLock(int bit)
{
    GLPP_ASSERT((UINT)bit <= _LOCK_LAST_BIT); // THE SPINLOCK BIT NUMBER IS INVALID
...
{
    GLPP_ASSERT((unsigned int)index < (unsigned int)m_sArgc); // INVALID COMMAND LINE ARGUMENT INDEX
    return m_sArgv[index];
}

_Ret_notnull_ Thread* Thread::getMainThread()
{
    GLPP_ASSERT(Thread::m_sMainThread != nullptr); // MAIN THREAD OBJECT IS UNINITIALIZED
    return Thread::m_sMainThread;
}

int Thread::getThreadId()
{
    GLPP_ASSERT(isThread()); // GLFW THREAD OBJECT HAS NO RUNNING THREAD
    return m_dwThreadId;
}
...
{
    GLPP_ASSERT(m_pWindow != nullptr); // NO OPENGL CONTEXT EXIST ON THE CALLING THREAD
    return m_pWindow;
}
#endif // #ifdef _DEBUG
} // namespace gl
