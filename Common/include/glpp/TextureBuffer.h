...
public:
	...
	{
		_bufferData(_tlsTextureBuffer(), GL_TEXTURE_BUFFER, (GLsizeiptr)dataStore.getSize(), dataStore.getData(0), (GLenum)usage);
	}
	...
	{
		_bufferSubData(_tlsTextureBuffer(), GL_TEXTURE_BUFFER, offset, size, data.getSubData((int)offset, (int)size));
	}
