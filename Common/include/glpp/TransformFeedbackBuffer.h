...
public:
	...
	{
#ifdef _DEBUG
		_object_assign((_Objects&)buffers, index, GL_TRANSFORM_FEEDBACK_BUFFER);
#else // #ifdef _DEBUG
	...
	void bindBuffer()
	{
		_buffer_rebind(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER);
	}
	...
	{
		_bindBufferBase(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, index);
	}
	...
	{
		_bindBufferRange(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, index, offset, size);
	}
	...
	{
		_bufferData(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, size, data, (GLenum)usage);
	}
	...
	{
		_bufferSubData(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, offset, size, data);
	}
	...
	{
		_flushMappedBufferRange(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, offset, length);
	}

	/*(3.0) Gets the access policy parameter value set while mapping the buffer object, the value of the access parameter enum passed to <MapBuffer>. If the buffer was mapped with <mapBufferRange>, the access policy is determined by translating the bits in that access parameter to one of the supported enums for <mapBuffer> as described in the OpenGL specification
	@return The buffer access policy enumerator*/
	BufferAccess getBufferAccess()
	{
		return (BufferAccess)_getBufferParameter(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, GL_BUFFER_ACCESS);
	}
	...
	{
		return (BufferFlags)_getBufferParameter(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, GL_BUFFER_ACCESS_FLAGS);
	}

	/*(3.0) Gets a parameter flag indicating whether the buffer object is currently mapped. The initial value is false
	@return The mapped flag*/
	GLboolean getBufferMapped()
	{
		return _getBufferParameter(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, GL_BUFFER_MAPPED);
	}
	...
	{
		return _getBufferPointer(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER);
	}

	/*(3.0) Gets the size parameter value of the buffer object parameter value, measured in bytes. The initial value is 0
	@return The size of the buffer object*/
	GLint getBufferSize()
	{
		return _getBufferParameter(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, GL_BUFFER_SIZE);
	}
	...
	{
		_getBufferSubData(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, offset, size, data.getSubData((int)offset, (int)size));
	}
	...
	{
		_getBufferSubData(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, offset, size, data);
	}

	/*(3.0) Gets the buffer object's usage pattern parameter value. The initial value is <StaticDraw>
	@return The buffer expected usage pattern enumerator*/
	BufferUsage getBufferUsage()
	{
		return (BufferUsage)_getBufferParameter(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, GL_BUFFER_USAGE);
	}
	...
	/*(3.0) Determines via API if the buffer object is currently bound to its target
	...
	{
		return _mapBuffer(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, (GLenum)access);
	}
	...
	{
		_buffer_unbind(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER);
	}
	...
	{
		return _unmapBuffer(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER);
	}
	...
	{
		_buffer_copy(_tlsTransformFeedbackBuffer(), _tlsCopyReadBuffer(), GL_COPY_READ_BUFFER);
	}

	/*(3.1) Explicitly binds buffer object to copy-write target, the previous binding is lost*/
	void bindCopyWriteBuffer()
	{
		_buffer_copy(_tlsTransformFeedbackBuffer(), _tlsCopyWriteBuffer(), GL_COPY_WRITE_BUFFER);
	}
	...
	{
		_copyBufferSubData(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, _tlsArrayBuffer(), GL_ARRAY_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, _tlsElementArrayBuffer(), GL_ELEMENT_ARRAY_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, _tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, _tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		GLuint* tls = _tlsTransformFeedbackBuffer();
		_copyBufferSubData(tls, GL_TRANSFORM_FEEDBACK_BUFFER, tls, GL_TRANSFORM_FEEDBACK_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, _tlsTextureBuffer(), GL_TEXTURE_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, _tlsUniformBuffer(), GL_UNIFORM_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyWriteBufferData(_tlsTransformFeedbackBuffer(), (GLsizeiptr)dataStore.getSize(), dataStore.getData(0), (GLenum)usage);
	}
	...
	{
		_copyWriteBufferData(_tlsTransformFeedbackBuffer(), size, data, (GLenum)usage);
	}
	...
	{
		return (GLsizeiptr)_getBufferParameter64(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, GL_BUFFER_MAP_LENGTH);
	}
	...
	{
		return (GLintptr)_getBufferParameter64(_tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, GL_BUFFER_MAP_OFFSET);
	}
#endif // #ifdef GL_VERSION_3_2
}; // class TransformFeedbackBuffer : public _Buffer
} // namespace gl
