...
#ifdef GLPP_NO_AFX_LAYOUT
		Thread s_NoAfxMainThread;
#endif // #ifdef GLPP_NO_AFX_LAYOUT
	} s_Data;
...
public:
	...
	/*Clears glpp object. If the last reference, removes the glpp object cell and if not duplicate object, destroys the associated window and its context*/
	void destroyWindow()
	{
	...
	/*Gets client API attribute value (default OpenglApi)
	@return The client API attribute enumerator*/
	...
	/*Gets context creation API attribute value (default NativeContextApi)
	@return The context creation API attribute enumerator*/
	ContextCreationApi getContextCreationApi()
	{
		return (ContextCreationApi)_getWindowAttrib(GLFW_CONTEXT_CREATION_API);
	}

	/*Gets context no error attribute flag (default false). Specifies whether errors should be generated by the context. If enabled, situations that would have generated errors instead cause undefined behavior
	@return Context no error attribute flag*/
	bool getContextNoError()
	{
		return (bool)_getWindowAttrib(GLFW_CONTEXT_NO_ERROR);
	}

	/*Gets context release behavior attribute value (default AnyReleaseBehavior)
	@return The context release behavior attribute enumerator*/
	...
	/*Gets context robustness attribute value (default NoRobustness)
	@return The context robustness attribute enumerator*/
	...
	/*Gets context version major attribute value (default 1). Indicate the client API major version of the window's context
	@return Context version major attribute value*/
	int getContextVersionMajor()
	{
		return _getWindowAttrib(GLFW_CONTEXT_VERSION_MAJOR);
	}

	/*Gets context version minor attribute value (default 0). Indicate the client API minor version of the window's context
	@return Context version minor attribute value*/
	int getContextVersionMinor()
	{
		return _getWindowAttrib(GLFW_CONTEXT_VERSION_MINOR);
	}

	/*Gets window cursor input mode value (defult CursorNormal). The cursor input mode provides several cursor modes for special forms of mouse motion input
	@return Cursor input mode enumerator*/
	...
	/*Gets decorated attribute flag (default true). Specifies whether the windowed mode window will have window decorations such as a border, a close widget, etc. An undecorated window will not be resizable by the user but will still allow the user to generate close events on some platforms. This attribute is ignored for full screen windows
	@return Decorated attribute flag*/
	bool getDecorated()
	{
		return (bool)_getWindowAttrib(GLFW_DECORATED);
	}

	/*Gets floating attribute flag (default false). Specifies whether the windowed mode window will be floating above other regular windows, also called topmost or always-on-top. This is intended primarily for debugging purposes and cannot be used to implement proper full screen windows. This attribute is ignored for full screen windows
	@return Floating attribute flag*/
	bool getFloating()
	{
		return (bool)_getWindowAttrib(GLFW_FLOATING);
	}

	/*Gets focused attribute flag (default true). Specifies whether the windowed mode window will be given input focus when created. This attribute is ignored for full screen and initially hidden windows
	@return Focused attribute flag*/
	bool getFocused()
	{
		return (bool)_getWindowAttrib(GLFW_FOCUSED);
	}

	/*Gets focus on show attribute flag (default true). Indicates whether the window will be given input focus when <showWindow> is called
	@return Focus on show attribute flag*/
	bool getFocusOnShow()
	{
	...
	/*Gets lock key mods input mode flag (default false). When this input mode is enabled, any callback that receives modifier bits will have the <KeyModifier::ModCapsLock> bit set if Caps Lock was on when the event occurred and the <KeyModifier::ModNumLock> bit set if Num Lock was on
	@return Lock key mods input mode flag*/
	bool getLockKeyMods()
	{
		return (bool)_getInputMode(GLFW_LOCK_KEY_MODS);
	}

	/*Gets maximized attribute flag (default false). Specifies whether the windowed mode window will be maximized when created. This attribute is ignored for full screen windows
	@return Maximized attribute flag*/
	bool getMaximized()
	{
	...
	/*Gets OpenGL debug context attribute flag (default false). Specifies whether the context should be created in debug mode, which may provide additional error and diagnostic reporting functionality
	@return OpenGL debug context attribute flag*/
	bool getOpenglDebugContext()
	{
		return (bool)_getWindowAttrib(GLFW_OPENGL_DEBUG_CONTEXT);
	}

	/*Gets OpenGL forward compatible attribute flag (default false). Specifies whether the OpenGL context should be forward-compatible, i.e. one where all functionality deprecated in the requested version of OpenGL is removed. This must only be used if the requested OpenGL version is 3.0 or above. If OpenGL ES is requested, this hint is ignored
	@return OpenGL forward compatible attribute flag*/
	bool getOpenglForwardCompat()
	{
		return (bool)_getWindowAttrib(GLFW_OPENGL_FORWARD_COMPAT);
	}

	/*Gets OpenGL profile attribute value (default OpenglAnyProfile)
	@return The OpenGL profile attribute enumerator*/
	OpenglProfile getOpenglProfile()
	{
		return (OpenglProfile)_getWindowAttrib(GLFW_OPENGL_PROFILE);
	}

	/*Gets raw mouse motion input mode flag (default false). When the cursor is disabled, raw (unscaled and unaccelerated) mouse motion can be enabled if available. Raw mouse motion is closer to the actual motion of the mouse across a surface. It is not affected by the scaling and acceleration applied to the motion of the desktop cursor. That processing is suitable for a cursor while raw motion is better for controlling for example a 3D camera. Because of this, raw mouse motion is only provided when the cursor is disabled
	@return Raw mouse motion input mode flag*/
	bool getRawMouseMotion()
	{
		return (bool)_getInputMode(GLFW_RAW_MOUSE_MOTION);
	}

	/*Gets resizable attribute flag (default true). Specifies whether the windowed mode window will be resizable by the user. The window will still be resizable using the glfwSetWindowSize function. This attribute is ignored for full screen and undecorated windows
	@return Resizable attribute flag*/
	bool getResizable()
	{
		return (bool)_getWindowAttrib(GLFW_RESIZABLE);
	}

	/*Gets window sticky keys input mode flag (default false). When sticky keys mode is enabled, the pollable state of a key will remain pressed (true) until the state of that key is polled with <getKey>. Once it has been polled, if a key release event had been processed in the meantime, the state will reset to released (false), otherwise it will remain pressed
	@return Sticky keys input mode flag*/
	bool getStickyKeys()
	{
		return (bool)(GLFW_STICKY_KEYS);
	}

	/*Gets window sticky mouse buttons input mode flag (default false). When sticky mouse buttons mode is enabled, the pollable state of a mouse button will pressed (true) until the state of that button is polled with <getMouseButton>. Once it has been polled, if a mouse button release event had been processed in the meantime, the state will reset to released (false), otherwise it will pressed
	@return Sticky mouse buttons input mode flag*/
	bool getStickyMouseButtons()
	{
		return (bool)_getInputMode(GLFW_STICKY_MOUSE_BUTTONS);
	}

	/*Gets transparent framebuffer attribute flag (default false). Indicates whether the specified window has a transparent framebuffer, i.e. the window contents is composited with the background using the window framebuffer alpha channel
	@return Transparent framebuffer attribute flag*/
	bool getTransparentFramebuffer()
	{
		return (bool)_getWindowAttrib(GLFW_TRANSPARENT_FRAMEBUFFER);
	}

	/*Gets visible attribute flag (default true). Specifies whether the windowed mode window will be initially visible. This attribute is ignored for full screen windows
	@return Visible attribute flag*/
	bool getVisible()
	{
	...
	/*Sets auto iconify writable attribute flag (default true). Specifies whether the full screen window will automatically iconify and restore the previous video mode on input focus loss. This attribute is ignored for windowed mode windows
	@param New auto iconify attribute flag*/
	...
	/*Sets window cursor input mode value (defult CursorNormal). The cursor input mode provides several cursor modes for special forms of mouse motion input
	@param New cursor input mode enumerator*/
	...
	/*Sets decorated writable attribute flag (default true). Specifies whether the windowed mode window will have window decorations such as a border, a close widget, etc. An undecorated window will not be resizable by the user but will still allow the user to generate close events on some platforms. This attribute is ignored for full screen windows
	@param New decorated attribute flag*/
	void setDecorated(bool flag)
	{
		_setWindowAttrib(GLFW_DECORATED, (int)flag);
	}

	/*Sets floating writable attribute flag (default false). Specifies whether the windowed mode window will be floating above other regular windows, also called topmost or always-on-top. This is intended primarily for debugging purposes and cannot be used to implement proper full screen windows. This attribute is ignored for full screen windows
	@param New floating attribute flag*/
	void setFloating(bool flag)
	{
		_setWindowAttrib(GLFW_FLOATING, (int)flag);
	}

	/*Sets focus on show writable attribute flag (default true). Specifies whether the window will be given input focus when <showWindow> is called
	@param New focus on show attribute flag*/
	void setFocusOnShow(bool flag)
	{
		_setWindowAttrib(GLFW_FOCUS_ON_SHOW, (int)flag);
	}

	/*Sets lock key mods input mode flag (default false). When this input mode is enabled, any callback that receives modifier bits will have the <KeyModifier::ModCapsLock> bit set if Caps Lock was on when the event occurred and the <KeyModifier::ModNumLock> bit set if Num Lock was on
	@param New lock key mods input mode flag*/
	void setLockKeyMods(bool flag)
	{
		_setInputMode(GLFW_LOCK_KEY_MODS, (int)flag);
	}

	/*Sets raw mouse motion input mode flag (default false). When the cursor is disabled, raw (unscaled and unaccelerated) mouse motion can be enabled if available. Raw mouse motion is closer to the actual motion of the mouse across a surface. It is not affected by the scaling and acceleration applied to the motion of the desktop cursor. That processing is suitable for a cursor while raw motion is better for controlling for example a 3D camera. Because of this, raw mouse motion is only provided when the cursor is disabled
	@param New raw mouse motion input mode flag*/
	void setRawMouseMotion(bool flag)
	{
		_setInputMode(GLFW_RAW_MOUSE_MOTION, (int)flag);
	}

	/*Sets window sticky keys input mode flag (default false). When sticky keys mode is enabled, the pollable state of a key will remain pressed (true) until the state of that key is polled with <getKey>. Once it has been polled, if a key release event had been processed in the meantime, the state will reset to released (false), otherwise it will remain pressed
	@param New sticky keys input mode flag*/
	void setStickyKeys(bool bFlag)
	{
		_setInputMode(GLFW_STICKY_KEYS, (int)bFlag);
	}

	/*Sets window sticky mouse buttons input mode flag (default false). When sticky mouse buttons mode is enabled, the pollable state of a mouse button will pressed (true) until the state of that button is polled with <getMouseButton>. Once it has been polled, if a mouse button release event had been processed in the meantime, the state will reset to released (false), otherwise it will pressed
	@param New sticky mouse buttons input mode flag*/
	void setStickyMouseButtons(bool flag)
	{
		_setInputMode(GLFW_STICKY_MOUSE_BUTTONS, (int)flag);
	}

	/*Sets resizable writable attribute flag (default true). Specifies whether the windowed mode window will be resizable by the user. The window will still be resizable using the <setWindowSize> function. This attribute is ignored for full screen and undecorated windows
	@param New resizable attribute flag*/
	...
	/*Sets mouse passthrough attribute flag (default false). Specifies whether the window is transparent to mouse input, letting any mouse events pass through to whatever window is behind it. This is only supported for undecorated windows. Decorated windows with this enabled will behave differently between platforms
	@param New mouse passthrough attribute flag*/
