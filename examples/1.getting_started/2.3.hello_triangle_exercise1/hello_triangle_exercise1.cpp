...
int main()
{
    ...
    float vertices[] = { // add a new set of vertices to form a second triangle (a total of 6 vertices); the vertex attribute configuration remains the same (still one 3-float position vector per vertex)
         // first triangle
        -0.9f, -0.5f, 0.0f,  // left
    ...
    while (!window.windowShouldClose) // render loop
    {
        ...
        gl::drawArrays(gl::PrimitiveMode::Triangles, 0, 6); // set the count to 6 since we're drawing 6 vertices now (2 triangles); not 3!
