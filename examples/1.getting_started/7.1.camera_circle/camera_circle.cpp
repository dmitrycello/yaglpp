...
int main()
{
    ...
    while (!window.windowShouldClose) // render loop
    {
        ...
        glm::mat4 view = glm::mat4(1.0f); // camera/view transformation: make sure to initialize matrix to identity matrix first
        float radius = 10.0f;
        float camX = sin((float)glfw::getTime() / 2.0f) * radius; // slow down the original speed
        float camZ = cos((float)glfw::getTime() / 2.0f) * radius;
