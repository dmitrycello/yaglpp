...
int main()
{
    ...
    window.setCursorPos((double)SCR_WIDTH / 2.0, (double)SCR_HEIGHT / 2.0); // center cursor
    ...
    texture2.textureWrapS = gl::TextureWrapMode::Repeat; // set e texture wrapping parameters
    ...
    ourShader.setInt("texture1", 0); // tell opengl for each sampler to which texture unit it belongs to
    ourShader.setInt("texture2", 1);

    while (!window.windowShouldClose) // render loop
    {
        window.processInput();
        gl::clearColor(0.2f, 0.2f, 0.3f, 1.0f); // same result: let's change background
        ...
        ourShader.setMat4("projection", projection); // pass projection matrix to shader (note that in this case it could change every frame)
        glm::mat4 view = window.camera.getViewMatrix(); // camera/view transformation
