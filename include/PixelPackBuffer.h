...
namespace gl {
/*(2.1) OpenGL pixel download buffer object class (PBO) derived from <_Buffer>*/
...
public:
	...
	PixelPackBuffer(Buffers& buffers, GLuint index)
	{
	...
	{
#ifdef _DEBUG
		_object_assign((_Objects&)buffers, index, GL_PIXEL_PACK_BUFFER);
#else // #ifdef _DEBUG
		_object_assign((_Objects&)buffers, index);
#endif // #ifdef _DEBUG
	}

	/*(2.1) Explicitly binds buffer object to its target*/
	void bindBuffer()
	{
		_buffer_rebind(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER);
	}
	...
	{
		_bufferData(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, (GLsizeiptr)dataStore.getSize(), dataStore.getData(), (GLenum)usage);
	}
	...
	{
		_bufferData(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, size, data, (GLenum)usage);
	}
	...
	{
		_bufferSubData(_tlsArrayBuffer(), GL_PIXEL_PACK_BUFFER, 0, data.getSize(), data.getData());
	}
	...
	{
		_bufferSubData(_tlsArrayBuffer(), GL_PIXEL_PACK_BUFFER, offset, size, data.getSubData((int)offset, (int)size));
	}
	...
	{
		_bufferSubData(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, offset, size, data);
	}

	/*(2.1) Gets the access policy parameter value set while mapping the buffer object, the value of the access parameter enum passed to <MapBuffer>. If the buffer was mapped with <mapBufferRange>, the access policy is determined by translating the bits in that access parameter to one of the supported enums for <mapBuffer> as described in the OpenGL specification
	@return The buffer access policy enumerator*/
	BufferAccess getBufferAccess()
	{
		return (BufferAccess)_getBufferParameter(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, GL_BUFFER_ACCESS);
	}

	/*(2.1) Gets a parameter flag indicating whether the buffer object is currently mapped. The initial value is false
	@return The mapped flag*/
	GLboolean getBufferMapped()
	{
		return _getBufferParameter(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, GL_BUFFER_MAPPED);
	}
	...
	{
		return _getBufferPointer(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER);
	}

	/*(2.1) Gets the size parameter value of the buffer object parameter value, measured in bytes. The initial value is 0
	@return The size of the buffer object*/
	GLint getBufferSize()
	{
		return _getBufferParameter(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, GL_BUFFER_SIZE);
	}
	...
	{
		_getBufferSubData(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, offset, size, data.getSubData((int)offset, (int)size));
	}
	...
	{
		_getBufferSubData(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, offset, size, data);
	}

	/*(2.1) Gets the buffer object's usage pattern parameter value. The initial value is <StaticDraw>
	@return The buffer expected usage pattern enumerator*/
	BufferUsage getBufferUsage()
	{
		return (BufferUsage)_getBufferParameter(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, GL_BUFFER_USAGE);
	}

	/*(2.1) Determines via API if the buffer object is currently bound to its target
	@return True if buffer object currently bound to its target, or false otherwise*/
	GLboolean isPixelPackBufferBinding()
	{
		return _object_id() == _getInteger(GL_PIXEL_PACK_BUFFER_BINDING);
	}
	...
	{
		return _mapBuffer(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, (GLenum)access);
	}
	...
	void shareBuffer(PixelPackBuffer& buffer)
	{
	...
	{
		_buffer_unbind(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER);
	}
	...
	{
		return _unmapBuffer(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER);
	}
	...
	{
		_flushMappedBufferRange(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, offset, length);
	}
	...
	@return Buffer access policy flag bitmask enumerator*/
	BufferFlags getBufferAccessFlags()
	{
		return (BufferFlags)_getBufferParameter(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, GL_BUFFER_ACCESS_FLAGS);
	}
	...
	{
		return _mapBufferRange(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, offset, length, (GLbitfield)access);
	}
	...
	{
		_buffer_copy(_tlsPixelPackBuffer(), _tlsCopyReadBuffer(), GL_COPY_READ_BUFFER);
	}

	/*(3.1) Explicitly binds buffer object to copy-write target, the previous binding is lost*/
	void bindCopyWriteBuffer()
	{
		_buffer_copy(_tlsPixelPackBuffer(), _tlsCopyWriteBuffer(), GL_COPY_WRITE_BUFFER);
	}
	...
	{
		_copyBufferSubData(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, _tlsArrayBuffer(), GL_ARRAY_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, _tlsElementArrayBuffer(), GL_ELEMENT_ARRAY_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		GLuint* tls = _tlsPixelPackBuffer();
		_copyBufferSubData(tls, GL_PIXEL_PACK_BUFFER, tls, GL_PIXEL_PACK_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, _tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, _tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, _tlsTextureBuffer(), GL_TEXTURE_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, _tlsUniformBuffer(), GL_UNIFORM_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyWriteBufferData(_tlsPixelPackBuffer(), (GLsizeiptr)dataStore.getSize(), dataStore.getData(0), (GLenum)usage);
	}
	...
	{
		_copyWriteBufferData(_tlsPixelPackBuffer(), size, data, (GLenum)usage);
	}
	...
	{
		return (GLsizeiptr)_getBufferParameter64(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, GL_BUFFER_MAP_LENGTH);
	}
	...
	{
		return (GLintptr)_getBufferParameter64(_tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, GL_BUFFER_MAP_OFFSET);
	}
