...
private:
	...
	static struct _SDATA {
		int iMcount;
		MonitorConnectedCallback* pConnected;
...
public:
	...
	{
		return m.fMonitorContentScaleX;
	}
	...
	{
		return m.fMonitorContentScaleY;
	}
	...
	{
		return m.iMonitorPhysicalWidth;
	}
	...
	{
		return m.iMonitorPhysicalHeight;
	}
	...
	{
		m.pSelected = getVideoMode(vindex);
	}
...
{
	return glfwGetGammaRamp(m.pMonitor);
}

inline void Monitor::getMonitorContentScale()
{
	glfwGetMonitorContentScale(m.pMonitor, &m.fMonitorContentScaleX, &m.fMonitorContentScaleY);
}

inline void Monitor::getMonitorPos()
{
	glfwGetMonitorPos(m.pMonitor, &m.iMonitorPosX, &m.iMonitorPosY);
}

inline void Monitor::getMonitorWorkarea()
{
	glfwGetMonitorWorkarea(m.pMonitor, &m.iMonitorPosX, &m.iMonitorPosY, &m.iMonitorWidth, &m.iMonitorHidth);
}

inline _Ret_notnull_ const GLFWvidmode* Monitor::getVideoMode()
{
	return glfwGetVideoMode(m.pMonitor);
}
...
{
	_select(glfwGetWindowMonitor(window.m.pWindow));
}

inline void Monitor::setGamma(float gamma)
{
	glfwSetGamma(m.pMonitor, gamma);
}

inline void Monitor::setGammaRamp(_In_ const GLFWgammaramp* ramp)
{
	glfwSetGammaRamp(m.pMonitor, ramp);
}
#endif // #ifndef _DEBUG
} // namespace glfw
