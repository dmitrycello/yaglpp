...
{
private:
	GLint _miLocation; // Uniform index
	GLuint _location();
...
{
	YAGLPP_ASSERT(_miLocation > -1); // OPENGL UNIFORM LOCATION INDEX IS INVALID
	return _miLocation;
}
...
{
	YAGLPP_ASSERT(location < (GLuint)_getInteger(GL_MAX_VERTEX_UNIFORM_COMPONENTS)); // OPENGL UNIFORM LOCATION INDEX IS INVALID
...
{
	glGetUniformfv(program._program_id(), _miLocation, params);
}

inline void Uniform::getUniform(Program& program, _Out_ GLint* params)
{
	glGetUniformiv(program._program_id(), _miLocation, params);
}
