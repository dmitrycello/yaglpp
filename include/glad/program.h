...
public:
	...
	{
		return Program(_getInteger(GL_CURRENT_PROGRAM));
	}
...
void Program::detachShaders()
{
	...
	if (iCount)
	{
		GLuint* pShaders = (GLuint*)_allocate(iCount * sizeof(GLuint), nullptr);
		glGetAttachedShaders(uId, iCount, &iCount, pShaders);
		for (GLint i = 0; i < iCount; i++)
		{
			glDetachShader(uId, pShaders[i]);
		}
		_deallocate(pShaders, nullptr);
	}
	YAGLPP_GLAD_ERROR;
}

GLboolean Program::isCurrentProgram() const
{
	GLuint uCurrent = (GLuint)_getInteger(GL_CURRENT_PROGRAM);
	return (uCurrent > 0) && (uCurrent == _object_id());
}
...
inline void Program::getTransformFeedbackVarying(GLuint index, _Out_ ActiveAttribInfo* info)
{
	glGetTransformFeedbackVarying(_program_id(), index, MAX_PATH, &info->length, &info->size, (GLenum*)&info->type, info->name);
}
...
#if defined (YAGLPP_VERSION_3_1) && defined (YAGLPP_INLINEG_IMPLEMENTATION)
