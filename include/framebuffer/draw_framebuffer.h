...
public:
	...
	void blitFramebuffer(ReadFramebuffer& source, GLint srcX0, GLint srcY0, GLint srcX1, GLint srcY1, GLint dstX0, GLint dstY0, GLint dstX1, GLint dstY1, BufferBitMask mask, TextureMagFilter filter)
	{
		bindFramebuffer();
		_blitFramebuffer((_Framebuffer&)source, GL_READ_FRAMEBUFFER, GL_READ_FRAMEBUFFER_BINDING, srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1, dstY1, mask, filter);
	}
	...
	static DrawFramebuffer getFramebufferBinding()
	{
		return DrawFramebuffer(_framebufferBinding(GL_DRAW_FRAMEBUFFER_BINDING));
	}
	...
	{
		return _object_id() == _framebufferBinding(GL_DRAW_FRAMEBUFFER_BINDING);
	}

	/*(3.0) Makes a reference of a framebuffer object, regardless of the source object's reference flag
	@param Specifies the source framebuffer object*/
	void referenceFramebuffer(const DrawFramebuffer& framebuffer)
	{
	...
	{
		_framebuffer_set(GL_DRAW_FRAMEBUFFER, GL_DRAW_FRAMEBUFFER_BINDING, bind);
	}
...
/*(3.0) Returns a reference DrawFramebuffer object currently bound to its target
@return The reference DrawFramebuffer object, or empty object*/
inline DrawFramebuffer getDrawFramebufferBinding()
{
	return DrawFramebuffer::getFramebufferBinding();
}
