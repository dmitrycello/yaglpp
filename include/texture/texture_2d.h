...
public:
	...
	static Texture2D getTextureBinding()
	{
		return Texture2D(_getInteger(GL_TEXTURE_BINDING_2D));
	}
	...
	{
		_texture_set(GL_TEXTURE_2D, GL_TEXTURE_BINDING_2D, bind);
	}
...
/*Returns a reference Texture2D object currently bound to its target
@return The reference Texture2D object, or empty object*/
inline Texture2D getTexture2DBinding()
{
	return Texture2D::getTextureBinding();
}
