...
public:
	...
	static Texture1D getTextureBinding()
	{
		return Texture1D(_getInteger(GL_TEXTURE_BINDING_1D));
	}
	...
	@param Specifies the source texture object*/
	void referenceTexture(const Texture1D& texture)
	{
	...
	{
		_texImage1D(GL_TEXTURE_1D, GL_TEXTURE_BINDING_1D, level, _stbGetFormat(image), image);
	}

	/*(2) Specifies a one-dimensional texture image initialized from the StbImage object with specified texture internal format. (2.1) Unbinds pixel unpack buffer from its target
	@param Specifies the StbImage object with loaded image
	@param Specifies the level-of-detail number
	...
	/*(3) Specifies a one-dimensional texture image initialized from client's memory. (2.1) Unbinds pixel unpack buffer from its target
	@param Specifies the level-of-detail number
	@param Specifies the texture internal formats
...
/*Returns a reference Texture1D object currently bound to its target
@return The reference Texture1D object, or empty object*/
inline Texture1D getTexture1DBinding()
{
	return Texture1D::getTextureBinding();
}
