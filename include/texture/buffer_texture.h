...
public:
	...
	static BufferTexture getTextureBinding()
	{
		return BufferTexture(_getInteger(GL_TEXTURE_BINDING_BUFFER));
	}
	...
	{
		_texture_set(GL_TEXTURE_BUFFER, GL_TEXTURE_BINDING_BUFFER, bind);
	}
...
/*(3.1) Returns a reference BufferTexture object currently bound to its target
@return The reference BufferTexture object, or empty object*/
inline BufferTexture getBufferTextureBinding()
{
	return BufferTexture::getTextureBinding();
}
