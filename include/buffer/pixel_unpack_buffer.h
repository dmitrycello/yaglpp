...
namespace gl {
/*(2.1) OpenGL pixel unpack buffer or pixel upload buffer object class (PBO) derived from <_Buffer>*/
class PixelUnpackBuffer : public _Buffer
{
private:
	friend class Buffers;
	PixelUnpackBuffer(GLint name) { _object_set(name); }
...
public:
	...
	PixelUnpackBuffer(const PixelUnpackBuffer& source)
	{
	...
	void bindBuffer()
	{
		_bindBuffer(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING);
	}
	...
	{
		_bufferData(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, (GLenum)usage, dataStore);
	}
	...
	{
		_bufferData(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, size, data, (GLenum)usage);
	}
	...
	{
		_bufferSubData(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, data);
	}
	...
	/*(2.1) Duplicates a buffer object, increasing its reference count. The reference source object is being copied
	@param Specifies the source buffer object*/
	void duplicateBuffer(const PixelUnpackBuffer& source)
	{
		_buffer_dup((_Object&)source);
	}
	...
	{
		return (BufferAccess)_getBufferParameter(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_BUFFER_ACCESS);
	}

	/*(2.1) Returns a reference buffer object currently bound to its target
	@return The reference buffer object, or empty object*/
	static PixelUnpackBuffer getBufferBinding()
	{
		return PixelUnpackBuffer(_getInteger(GL_PIXEL_UNPACK_BUFFER_BINDING));
	}
	...
	{
		return _getBufferParameter(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_BUFFER_MAPPED);
	}
	...
	{
		return _getBufferPointer(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING);
	}
	...
	GLint getBufferSize()
	{
		return _getBufferParameter(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_BUFFER_SIZE);
	}
	...
	{
		_getBufferSubData(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, offset, size, data);
	}
	...
	{
		return (BufferUsage)_getBufferParameter(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_BUFFER_USAGE);
	}
	...
	{
		return _object_binding(GL_PIXEL_UNPACK_BUFFER_BINDING);
	}

	/*(2.1) Checks if the source buffer object is referencing the same OpenGL object
	@param Specifies the source buffer object
	@return True if duplicate object*/
	GLboolean isDuplicate(const PixelUnpackBuffer& source) const
	{
	...
	/*(2.1) Creates a thread-safe reference object from the source buffer object
	@param Specifies the source buffer object*/
	void referBuffer(const PixelUnpackBuffer& source)
	{
	...
	{
		_buffer_set(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, bind);
	}

	/*(2.1) Explicitly unbinds specified buffer object from its target. Does nothing if specified buffer is not bound*/
	void unbindBuffer()
	{
		_unbindBuffer(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING);
	}
	...
	{
		_unbindTarget(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING);
	}
	...
	{
		return _unmapBuffer(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING);
	}
	...
	{
		_flushMappedBufferRange(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, offset, length);
	}
	...
	{
		return (BufferFlags)_getBufferParameter(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_BUFFER_ACCESS_FLAGS);
	}
	...
	{
		return _mapBufferRange(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, offset, length, (GLbitfield)access);
	}
	...
	{
		_copyBufferSubData(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_ARRAY_BUFFER, GL_ARRAY_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_ELEMENT_ARRAY_BUFFER, GL_ELEMENT_ARRAY_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_PIXEL_PACK_BUFFER, GL_PIXEL_PACK_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_UNIFORM_BUFFER, GL_UNIFORM_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		return (GLsizeiptr)_getBufferParameter64(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_BUFFER_MAP_LENGTH);
	}
	...
	{
		return (GLintptr)_getBufferParameter64(GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, GL_BUFFER_MAP_OFFSET);
	}
...
/*(2.1) Returns a reference PixelUnpackBuffer object currently bound to its target
@return The reference PixelUnpackBuffer object, or empty object*/
inline PixelUnpackBuffer getPixelUnpackBufferBinding()
{
	return PixelUnpackBuffer::getBufferBinding();
}

/*(2.1) Explicitly unbinds any buffer object of specified type bound to its target. Does nothing if no such buffer is bound*/
inline void unbindPixelUnpackBuffer()
{
	PixelUnpackBuffer::unbindTarget();
}
} // namespace gl
