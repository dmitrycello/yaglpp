...
#ifndef YAGLPP_TRANSFORM_FEEDBACK_BUFFER_H
#define YAGLPP_TRANSFORM_FEEDBACK_BUFFER_H

#ifdef YAGLPP_VERSION_3_0
namespace gl {
/*(3.0) OpenGL transform feedback buffer object class (TFO) derived from <Buffers>*/
class TransformFeedbackBuffer : public Buffers
{
public:
	using Buffers::Buffers;
	/*(3.0) (1) Constructs default array buffer object*/
	TransformFeedbackBuffer() : Buffers() {}

	/*(3.0) (2) Constructs array buffer and initializes buffer object's data store with <Data>*/
	TransformFeedbackBuffer(DataStore& dataStore, Usage usage) : Buffers()
	{
		buffer_create(GL_TRANSFORM_FEEDBACK_BUFFER, (GLenum)usage, dataStore);
	}

	/*(3.0) (3) Constructs array buffer and initializes buffer object's data store with <Data>*/
	TransformFeedbackBuffer(GLsizeiptr size, _Pre_maybenull_ const void* data, Usage usage) : Buffers()
	{
		buffer_create(GL_TRANSFORM_FEEDBACK_BUFFER, size, data, (GLenum)usage);
	}

	/*(3.0) Binds buffer object to its target. Does nothing if specified object is bound*/
	void Bind()
	{
		object_bind(glBindBuffer, GL_TRANSFORM_FEEDBACK_BUFFER);
	}
	...
	{
		bind_buffer_base(GL_TRANSFORM_FEEDBACK_BUFFER, index);
	}
	...
	{
		bind_buffer_range(GL_TRANSFORM_FEEDBACK_BUFFER, index, offset, size);
	}

	/*(3.0) (1) Creates and initializes buffer object's data store with the valid data DataStore object
	...
	{
		buffer_data(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, (GLenum)usage, dataStore);
	}
	...
	{
		flush_mapped_buffer_range(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, offset, length);
	}

	/*(3.0) Generates OpenGL buffer object name in an empty buffer class object, and bind it to its target in one function call*/
	void GenBind()
	{
		object_gen_bind(glGenBuffers, glBindBuffer, GL_TRANSFORM_FEEDBACK_BUFFER);
	}

	/*(3.0) Gets the access policy parameter value set while mapping the buffer object, the value of the access parameter enum passed to <Map>. If the buffer was mapped with <MapRange>, the access policy is determined by translating the bits in that access parameter to one of the supported enums for <Map> as described in the OpenGL specification. MSVC getter for <access> property
	@return The buffer access policy enumerator*/
	Access GetAccess()
	{
		return (Access)get_buffer_parameter(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_BUFFER_ACCESS);
	}
	...
	{
		return (AccessFlags)get_buffer_parameter(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_BUFFER_ACCESS_FLAGS);
	}
	...
	{
		return get_buffer_pointer(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING);
	}

	/*(3.0) Gets the size parameter value of the buffer object parameter value, measured in bytes. The initial value is 0. MSVC getter for <size> property
	@return The size of the buffer object*/
	GLint GetSize()
	{
		return get_buffer_parameter(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_BUFFER_SIZE);
	}
	...
	{
		get_buffer_sub_data(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, offset, size, data);
	}
	...
	/*(3.0) Gets the buffer object's usage pattern parameter value. The initial value is <StaticDraw>. MSVC getter for <usage> property
	@return The buffer expected usage pattern enumerator*/
	Usage GetUsage()
	{
		return (Usage)get_buffer_parameter(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_BUFFER_USAGE);
	}

	/*(3.0) Determines if the buffer object is currently bound to its target. MSVC getter for <binding> property
	@return True if buffer object currently bound to its target, or false otherwise*/
	GLboolean IsBinding() const
	{
		return object_binding(GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, Get());
	}

	/*(3.0) Gets a parameter flag indicating whether the buffer object is currently mapped. The initial value is false. MSVC getter for <mapped> property
	@return The mapped flag*/
	GLboolean IsMapped()
	{
		return get_buffer_parameter(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_BUFFER_MAPPED);
	}
	...
	{
		return map_buffer(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, (GLenum)access);
	}
	...
	{
		return map_buffer_range(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, offset, length, (GLbitfield)access);
	}
	...
	{
		buffer_sub_data(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, data);
	}
	...
	{
		buffer_sub_data(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, offset, size, data);
	}
	...
	/*(3.0) Unbinds any buffer object bound to particular target. Does nothing if no such object is bound*/
	static void Unbind()
	{
	...
	GLboolean Unmap()
	{
		return unmap_buffer(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING);
	}
	...
	/*(3.0) Read-only property for buffer binding flag*/
	...
	{
		copy_buffer_sub_data(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, readOffset, writeOffset, size);
	}
	...
	{
		copy_buffer_sub_data(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_ARRAY_BUFFER, GL_ARRAY_BUFFER_BINDING, ((Buffers&)writeBuffer).Get(), readOffset, writeOffset, size);
	}
	...
	{
		copy_buffer_sub_data(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_ELEMENT_ARRAY_BUFFER, GL_ELEMENT_ARRAY_BUFFER_BINDING, ((Buffers&)writeBuffer).Get(), readOffset, writeOffset, size);
	}
	...
	{
		copy_buffer_sub_data(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_PIXEL_PACK_BUFFER, GL_PIXEL_PACK_BUFFER_BINDING, ((Buffers&)writeBuffer).Get(), readOffset, writeOffset, size);
	}
	...
	{
		copy_buffer_sub_data(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, ((Buffers&)writeBuffer).Get(), readOffset, writeOffset, size);
	}
	...
	{
		copy_buffer_sub_data(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, ((Buffers&)writeBuffer).Get(), readOffset, writeOffset, size);
	}
	...
	{
		copy_buffer_sub_data(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_UNIFORM_BUFFER, GL_UNIFORM_BUFFER_BINDING, ((Buffers&)writeBuffer).Get(), readOffset, writeOffset, size);
	}
	...
	{
		return get_buffer_parameter64(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_BUFFER_MAP_LENGTH);
	}
	...
	{
		return get_buffer_parameter64(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_BUFFER_MAP_OFFSET);
	}
	...
	GLint64 GetSize64()
	{
		return get_buffer_parameter64(GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, GL_BUFFER_SIZE);
	}
...
#endif // #ifndef YAGLPP_TRANSFORM_FEEDBACK_BUFFER_H
