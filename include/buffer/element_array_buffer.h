...
private:
	...
	ElementArrayBuffer(GLint name) { _object_set(name); }
...
public:
	...
	ElementArrayBuffer(const ElementArrayBuffer& source)
	{
	...
	void duplicateBuffer(const ElementArrayBuffer& source)
	{
		_buffer_dup((_Object&)source);
	}
	...
	static ElementArrayBuffer getBufferBinding()
	{
		return ElementArrayBuffer(_getInteger(GL_ELEMENT_ARRAY_BUFFER_BINDING));
	}
	...
	{
		return _object_binding(GL_ELEMENT_ARRAY_BUFFER_BINDING);
	}

	/*Checks if the source buffer object is referencing the same OpenGL object
	@param Specifies the source buffer object
	@return True if duplicate object*/
	GLboolean isDuplicate(const ElementArrayBuffer& source) const
	{
	...
	@param Specifies the source buffer object*/
	void referBuffer(const ElementArrayBuffer& source)
	{
	...
	{
		_buffer_set(GL_ELEMENT_ARRAY_BUFFER, GL_ELEMENT_ARRAY_BUFFER_BINDING, bind);
	}
...
/*Returns a reference ElementArrayBuffer object currently bound to its target
@return The reference ElementArrayBuffer object, or empty object*/
inline ElementArrayBuffer getElementArrayBufferBinding()
{
	return ElementArrayBuffer::getBufferBinding();
}
