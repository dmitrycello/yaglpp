...
public:
	...
	{
		_bufferData(GL_UNIFORM_BUFFER, GL_UNIFORM_BUFFER_BINDING, (GLenum)usage, dataStore);
	}
	...
	{
		_bufferSubData(GL_UNIFORM_BUFFER, GL_UNIFORM_BUFFER_BINDING, data);
	}
	...
	static UniformBuffer getBufferBinding()
	{
		return UniformBuffer(_getInteger(GL_UNIFORM_BUFFER_BINDING));
	}
	...
	{
		_buffer_set(GL_UNIFORM_BUFFER, GL_UNIFORM_BUFFER_BINDING, bind);
	}
...
/*(3.1) Returns a reference UniformBuffer object currently bound to its target
@return The reference UniformBuffer object, or empty object*/
inline UniformBuffer getUniformBufferBinding()
{
	return UniformBuffer::getBufferBinding();
}
