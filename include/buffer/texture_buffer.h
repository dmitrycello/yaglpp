...
private:
	...
	TextureBuffer(GLint name) { _object_set(name); }
...
public:
	...
	void bindBuffer()
	{
		_bindBuffer(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING);
	}
	...
	{
		_bufferData(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, (GLenum)usage, dataStore);
	}
	...
	{
		_bufferData(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, size, data, (GLenum)usage);
	}
	...
	{
		_bufferSubData(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, data);
	}
	...
	{
		_copyBufferSubData(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_ARRAY_BUFFER, GL_ARRAY_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_ELEMENT_ARRAY_BUFFER, GL_ELEMENT_ARRAY_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_PIXEL_PACK_BUFFER, GL_PIXEL_PACK_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_PIXEL_UNPACK_BUFFER, GL_PIXEL_UNPACK_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
 	}
	...
	{
		_copyBufferSubData(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_TRANSFORM_FEEDBACK_BUFFER, GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_UNIFORM_BUFFER, GL_UNIFORM_BUFFER_BINDING, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	void duplicateBuffer(const TextureBuffer& source)
	{
		_buffer_dup((_Object&)source);
	}
	...
	{
		_flushMappedBufferRange(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, offset, length);
	}
	...
	{
		return (BufferAccess)_getBufferParameter(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_BUFFER_ACCESS);
	}
	...
	{
		return (BufferFlags)_getBufferParameter(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_BUFFER_ACCESS_FLAGS);
	}

	/*(3.1) Returns a reference buffer object currently bound to its target
	@return The reference buffer object, or empty object*/
	static TextureBuffer getBufferBinding()
	{
		return TextureBuffer(_getInteger(GL_TEXTURE_BUFFER_BINDING));
	}
	...
	{
		return _getBufferParameter(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_BUFFER_MAPPED);
	}
	...
	{
		return _getBufferPointer(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING);
	}
	...
	GLint getBufferSize()
	{
		return _getBufferParameter(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_BUFFER_SIZE);
	}
	...
	{
		_getBufferSubData(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, offset, size, data);
	}
	...
	{
		return (BufferUsage)_getBufferParameter(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_BUFFER_USAGE);
	}
	...
	{
		return _mapBuffer(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, (GLenum)access);
	}
	...
	@param Specifies the source buffer object*/
	void referBuffer(const TextureBuffer& source)
	{
	...
	{
		_buffer_set(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, bind);
	}

	/*(3.1) Explicitly unbinds specified buffer object from its target. Does nothing if specified buffer is not bound*/
	void unbindBuffer()
	{
		_unbindBuffer(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING);
	}
	...
	{
		_unbindTarget(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING);
	}
	...
	{
		return _unmapBuffer(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING);
	}
	...
	{
		return (GLsizeiptr)_getBufferParameter64(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_BUFFER_MAP_LENGTH);
	}
	...
	{
		return (GLintptr)_getBufferParameter64(GL_TEXTURE_BUFFER, GL_TEXTURE_BUFFER_BINDING, GL_BUFFER_MAP_OFFSET);
	}
...
/*(3.1) Returns a reference TextureBuffer object currently bound to its target
@return The reference TextureBuffer object, or empty object*/
inline TextureBuffer getTextureBufferBinding()
{
	return TextureBuffer::getBufferBinding();
}

/*(3.1) Explicitly unbinds any buffer object of specified type bound to its target. Does nothing if no such buffer is bound*/
inline void unbindTextureBuffer()
{
	TextureBuffer::unbindTarget();
}
} // namespace gl
