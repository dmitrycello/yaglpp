...
void _dropFree(int pathcount, const char** paths)
{
	...
	{
		YAGLPP_FREE((void*)paths[i]);
	}
	YAGLPP_FREE((void*)paths);
}
...
void Window::_dropCallback(GLFWwindow* window, int pathcount, const char** paths)
{
	...
	const char** pPaths = (const char**)YAGLPP_MALLOC(sizeof(WPARAM) * pathcount);
	for (int i = 0; i < pathcount; i++)
	{
...
void Window::setClipboardString(_In_z_ const char* string)
{
	...
	else
	{
		...
		{
			YAGLPP_FREE(pString); // Cleanup if message blocked
		}
	}
}
...
bool Window::translateMessage(_Inout_ Message* msg)
{
	...
	if (isMainThreadCurrent())
	{
		...
		if (pThread != nullptr)
		{
			...
			{
				YAGLPP_FREE(msg->x.p); // Cleanup if message blocked
			}
		}
		return true;
	}
	...
	{
		YAGLPP_FREE(msg->x.p); // Cleanup string
	}
	return true;
}
