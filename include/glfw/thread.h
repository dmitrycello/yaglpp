...
{
    return YAGLPP_MALLOC(size);
}

void* _reallocate(void* block, size_t size, void* user)
{
    return YAGLPP_REALLOC(block, size);
}
...
bool Thread::_SDATA::initialize(Thread* thread)
{
    ...
    dEventsTimeout = 0.0;
    pMainThread = thread;
    pLocks = (LONG*)YAGLPP_CALLOC((YAGLPP_INIT_SIZE >> 5) * sizeof(LONG));
    pEvents = (HANDLE*)YAGLPP_CALLOC(YAGLPP_INIT_SIZE * sizeof(HANDLE));
    ...
    WCHAR* pCmdW = (WCHAR*)YAGLPP_MALLOC(iLen * sizeof(WCHAR));
    ...
    YAGLPP_FREE(pCmdW);
    pArgv = (char**)YAGLPP_MALLOC(iArgc * sizeof(char*));
    for (int i = 0; i < iArgc; i++)
    {
        iLen = (int)wcslen(pArrW[i]) + 1;
        pArgv[i] = (char*)YAGLPP_MALLOC(sizeof(char) * iLen);
...
int Thread::_SDATA::syncRef()
{
    ...
    if (iEvent == iESize)
    {
        ...
        pLocks = (LONG*)YAGLPP_CALLOC(iNewLock * sizeof(LONG));
        pEvents = (HANDLE*)YAGLPP_CALLOC(iESize * sizeof(HANDLE));
        ...
        YAGLPP_FREE((void*)pOldLocks);
        YAGLPP_FREE((void*)pOldEvents);
    }
...
void Thread::_SDATA::terminate()
{
    ...
    YAGLPP_FREE((void*)pLocks);
    YAGLPP_FREE((void*)pEvents);
...
bool Thread::_message(const Message* msg)
{
    ...
    {
        _m.iSize <<= 1; // x2
        _m.pQueue = (Message*)YAGLPP_REALLOC(_m.pQueue, _m.iSize * sizeof(Message));
    }
...
Thread::Thread()
{
    ...
    _m.pQueue = (Message*)YAGLPP_MALLOC(YAGLPP_INIT_SIZE * sizeof(Message));
    _m.iRef = syncRef();
}
