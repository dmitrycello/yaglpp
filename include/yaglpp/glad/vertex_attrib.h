...
namespace gl {
/*OpenGL vertex attribute class derived from <Indices>*/
class VertexAttrib : public Indices
{
public:
	using Indices::Indices;
	enum class Size : GLsizei;
	enum class Type : GLenum;
	/*(1) Constructs unassigned vertex attribute*/
	VertexAttrib() : Indices() {}

	/*(2) Constructs vertex attribute with <SetLocation>*/
	VertexAttrib(GLuint location) : Indices()
	{
		SetLocation(location);
	}

	/*(3) Constructs vertex attribute with <GetLocation>*/
	VertexAttrib(Program& program, _In_z_ const GLchar* name)
	{
		GetLocation(program, name);
	}

	/*Allocates an array of vertex attributes of a given size, and initialize their location values. Must be deallocated with <VertexAttrib::Dealloc> after use
	@param The number of the array elements
	@return The pointer to allocated array*/
	static VertexAttrib* Alloc(GLsizei n)
	{
		return (VertexAttrib*)indices_alloc(n);
	}
	...
	@param [in] Specifies a null terminated string containing the name of the vertex shader attribute variable to which object is to be bound*/
	void BindMatrix2(Program& program, VertexAttrib& mat2, GLuint index, _In_z_ const GLchar* name);
	...
	@param Specifies the index of the generic vertex attribute to be bound as the first column of the matrix
	@param [in] Specifies a null terminated string containing the name of the vertex shader attribute variable to which object is to be bound*/
	void BindMatrix3(Program& program, VertexAttrib& mat2, VertexAttrib& mat3, GLuint index, _In_z_ const GLchar* name);
	...
	@param Specifies the index of the generic vertex attribute to be bound as the first column of the matrix
	@param [in] Specifies a null terminated string containing the name of the vertex shader attribute variable to which object is to be bound*/
	void BindMatrix4(Program& program, VertexAttrib& mat2, VertexAttrib& mat3, VertexAttrib& mat4, GLuint index, _In_z_ const GLchar* name);

	/*Deletes previously allocated array of vertex attributes, and deallocates its memory block
	@param The pointer to allocated array*/
	static void Dealloc(VertexAttrib* uniforms)
	{
...
void VertexAttrib::BindLocation(Program& program, GLuint index, _In_z_ const GLchar* name)
{
	...
	YAGLPP_GLAD_ERROR;
	indices_location(index);
}

void VertexAttrib::BindMatrix2(Program& program, VertexAttrib& mat2, GLuint index, _In_z_ const GLchar* name)
{
	// INVALID PROGRAM OBJECT
	YAGLPP_ASSERT(program.IsProgram());

	// VERTEX ATTRIBUTE LOCATION VALUE IS UNSUPPORTED
	YAGLPP_ASSERT((index + 1) < GetMaxVertexAttribs());

	glBindAttribLocation(program.Get(), index, name);
	YAGLPP_GLAD_ERROR;
	indices_location(index);
	mat2.indices_location(index + 1);
}

void VertexAttrib::BindMatrix3(Program& program, VertexAttrib& mat2, VertexAttrib& mat3, GLuint index, _In_z_ const GLchar* name)
{
	// INVALID PROGRAM OBJECT
	YAGLPP_ASSERT(program.IsProgram());

	// VERTEX ATTRIBUTE LOCATION VALUE IS UNSUPPORTED
	YAGLPP_ASSERT((index + 2) < GetMaxVertexAttribs());

	glBindAttribLocation(program.Get(), index, name);
	YAGLPP_GLAD_ERROR;
	indices_location(index);
	mat2.indices_location(index + 1);
	mat2.indices_location(index + 2);
}

void VertexAttrib::BindMatrix4(Program& program, VertexAttrib& mat2, VertexAttrib& mat3, VertexAttrib& mat4, GLuint index, _In_z_ const GLchar* name)
{
	// INVALID PROGRAM OBJECT
	YAGLPP_ASSERT(program.IsProgram());

	// VERTEX ATTRIBUTE LOCATION VALUE IS UNSUPPORTED
	YAGLPP_ASSERT((index + 3) < GetMaxVertexAttribs());
...
{
	GLint iResult;
	glGetVertexAttribiv(indices_location(), ePname, &iResult);
	YAGLPP_GLAD_ERROR;
	return iResult;
}
...
{
	glDisableVertexAttribArray(indices_location());
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::EnableArray()
{
	glEnableVertexAttribArray(indices_location());
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::GetCurrent(_Out_writes_(4) GLint* params)
{
	glGetVertexAttribiv(indices_location(), GL_CURRENT_VERTEX_ATTRIB, params);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::GetCurrent(_Out_writes_(4) GLfloat* params)
{
	glGetVertexAttribfv(indices_location(), GL_CURRENT_VERTEX_ATTRIB, params);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::GetCurrent(_Out_writes_(4) GLdouble* params)
{
	glGetVertexAttribdv(indices_location(), GL_CURRENT_VERTEX_ATTRIB, params);
	YAGLPP_GLAD_ERROR;
}
...
void VertexAttrib::GetLocation(Program& program, _In_z_ const GLchar* name)
{
	...
	indices_location(glGetAttribLocation(program.Get(), name));
	YAGLPP_GLAD_ERROR;
}
...
{
	void* pData;
	glGetVertexAttribPointerv(indices_location(), GL_VERTEX_ATTRIB_ARRAY_POINTER, &pData);
	YAGLPP_GLAD_ERROR;
...
{
	GLint i; glGetVertexAttribiv(indices_location(), ePname, &i); return i;
}
...
{
	void* p; glGetVertexAttribPointerv(indices_location(), GL_VERTEX_ATTRIB_ARRAY_POINTER, &p); return (GLintptr)p;
}
...
{
	glGetVertexAttribIiv(indices_location(), GL_CURRENT_VERTEX_ATTRIB, params);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::GetCurrentI(_Out_writes_(4) GLuint* params)
{
	glGetVertexAttribIuiv(indices_location(), GL_CURRENT_VERTEX_ATTRIB, params);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::PointerI(SizeI size, TypeI type, GLsizei stride, GLintptr offset)
{
	glVertexAttribIPointer(indices_location(), (GLint)size, (GLenum)type, stride, (void*)offset);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(GLint x)
{
	glVertexAttribI1i(indices_location(), x);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(GLint x, GLint y)
{
	glVertexAttribI2i(indices_location(), x, y);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(GLint x, GLint y, GLint z)
{
	glVertexAttribI3i(indices_location(), x, y, z);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(GLint x, GLint y, GLint z, GLint w)
{
	glVertexAttribI4i(indices_location(), x, y, z, w);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(GLuint x)
{
	glVertexAttribI1ui(indices_location(), x);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(GLuint x, GLuint y)
{
	glVertexAttribI2ui(indices_location(), x, y);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(GLuint x, GLuint y, GLuint z)
{
	glVertexAttribI3ui(indices_location(), x, y, z);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(GLuint x, GLuint y, GLuint z, GLuint w)
{
	glVertexAttribI4ui(indices_location(), x, y, z, w);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(_In_ const GLint* value)
{
	glVertexAttribI1iv(indices_location(), value);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(_In_ const glm::ivec2* value)
{
	glVertexAttribI2iv(indices_location(), (GLint*)value);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(_In_ const glm::ivec3* value)
{
	glVertexAttribI3iv(indices_location(), (GLint*)value);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(_In_ const glm::ivec4* value)
{
	glVertexAttribI4iv(indices_location(), (GLint*)value);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(_In_ const GLuint* value)
{
	glVertexAttribI1uiv(indices_location(), value);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(_In_ const glm::uvec2* value)
{
	glVertexAttribI2uiv(indices_location(), (GLuint*)value);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(_In_ const glm::uvec3* value)
{
	glVertexAttribI3uiv(indices_location(), (GLuint*)value);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(_In_ const glm::uvec4* value)
{
	glVertexAttribI4uiv(indices_location(), (GLuint*)value);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(_In_ const glm::cvec4* value)
{
	glVertexAttribI4bv(indices_location(), (GLbyte*)value);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(_In_ const glm::ucvec4* value)
{
	glVertexAttribI4ubv(indices_location(), (GLubyte*)value);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(_In_ const glm::svec4* value)
{
	glVertexAttribI4sv(indices_location(), (GLshort*)value);
	YAGLPP_GLAD_ERROR;
}

void VertexAttrib::SetI(_In_ const glm::usvec4* value)
{
	glVertexAttribI4usv(indices_location(), (GLushort*)value);
	YAGLPP_GLAD_ERROR;
}
