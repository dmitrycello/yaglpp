...
public:
	...
	/*Allocates an initialized array of shader objects of a given size. Must be deallocated with <Shaders::Dealloc> after use
	@param The number of the array elements
	@return The pointer to allocated array*/
	static Shaders* Alloc(GLsizei n)
	{
		return (Shaders*)yaglpp_callocate(n * sizeof(GLuint), nullptr);
	}

	/*Deletes previously allocated shader objects, and deallocates its memory block
	@param The pointer to allocated array*/
	static void Dealloc(Shaders* shaders);
	...
	{
		return shader_get(GL_COMPILE_STATUS);
	}
	...
	{
		return shader_get(GL_DELETE_STATUS);
	}
	...
	{
		return shader_get(GL_INFO_LOG_LENGTH);
	}
	...
	{
		return shader_get(GL_SHADER_SOURCE_LENGTH);
	}
	...
	Type GetType()
	{
		return (Type)shader_get(GL_SHADER_TYPE);
	}
...
protected:
	...
	void source(GLenum type, GLsizei count, const GLchar** string, const GLint* length);
	void source_from_file(GLenum type, const char* filepath);
	void source_from_memory(GLenum type, unsigned char* data, unsigned int size);

#ifdef YAGLPP_MSVC
	void source_from_resource(GLenum type, int rcid);
#endif // #ifdef YAGLPP_MSVC
...
void Shaders::source(GLenum type, GLsizei count, const GLchar** string, const GLint* length)
{
	object_shader(type);
...
void Shaders::source_from_file(GLenum type, const char* filepath)
{
	object_shader(type);
...
void Shaders::source_from_memory(GLenum type, unsigned char* data, unsigned int size)
{
	object_shader(type);
...
void Shaders::Dealloc(Shaders* shaders)
{
	GLsizei n = (GLsizei)yaglpp_block_size(shaders, nullptr) / sizeof(GLuint);
	for (GLsizei i = 0; i < n; i++)
	{
		if (glIsShader(((GLuint*)shaders)[i]))
		{
...
void Shaders::source_from_resource(GLenum type, int rcid)
{
	object_shader(type);
...
GLint Shaders::shader_get(GLenum pname)
{
	GLint iResult;
...
inline GLint Shaders::shader_get(GLenum pname)
{
