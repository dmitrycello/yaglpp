...
public:
	...
	/*Direct object's new operator is not allowed. The object's <Alloc> must be used if applicable*/
	void* operator new(std::size_t) = delete;

	/*Direct object's new[] operator is not allowed. The object's <Alloc> must be used if applicable*/
	void* operator new[](std::size_t) = delete;

	/*Direct object's delete operator is not allowed. The object's <Dealloc> must be used if applicable*/
...
void Objects::object_dealloc(PFNDELETE pfnDelete, GLuint* objects)
{
	...
	pfnDelete(n, objects);
	YAGLPP_GLAD_ERROR;
	yaglpp_deallocate(objects, nullptr);
}
