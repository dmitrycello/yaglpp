...
public:
	...
	/*Allocates an initialized array of program objects of a given size. Must be deallocated with <Program::Dealloc> after use
	@param The number of the array elements
	@return The pointer to allocated array*/
	static Program* Alloc(GLsizei n)
	{
		return (Program*)yaglpp_callocate(n * sizeof(GLuint), nullptr);
	}
	...
	static void Dealloc(Program* programs);
...
protected:
	void auto_delete() { if (IsProgram()) Delete(); }
...
void Program::Dealloc(Program* programs)
{
	GLsizei n = (GLsizei)yaglpp_block_size(programs, nullptr) / sizeof(GLuint);
	for (GLsizei i = 0; i < n; i++)
	{
		if (glIsShader(((GLuint*)programs)[i]))
		{
			glDeleteShader(((GLuint*)programs)[i]);
		}
	}
	YAGLPP_GLAD_ERROR;
	YAGLPP_FREE(programs);
}
