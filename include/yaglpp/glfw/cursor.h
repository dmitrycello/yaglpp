...
public:
	...
	Cursor(StbImage& stb, int xhot, int yhot) : Cursor()
	{
		Create(stb, xhot, yhot);
	}
...
inline void Cursor::Create(Shape shape)
{
	m_Cursor = glfwCreateStandardCursor((int)shape);
}

inline void Cursor::Create(_In_ const GLFWimage* image, int xhot, int yhot)
{
	m_Cursor = glfwCreateCursor(image, xhot, yhot);
}
