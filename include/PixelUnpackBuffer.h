...
namespace gl {
/*(2.1) OpenGL pixel upload buffer object class (PBO) derived from <_Buffer>*/
class PixelUnpackBuffer : public _Buffer
{
public:
	...
	{
#ifdef _DEBUG
		_object_assign((_Objects&)buffers, index, GL_PIXEL_UNPACK_BUFFER);
#else // #ifdef _DEBUG
	...
	void bindBuffer()
	{
		_buffer_rebind(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER);
	}
	...
	{
		_bufferData(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, (GLsizeiptr)dataStore.getSize(), dataStore.getData(), (GLenum)usage);
	}
	...
	{
		_bufferData(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, size, data, (GLenum)usage);
	}
	...
	{
		_bufferSubData(_tlsArrayBuffer(), GL_PIXEL_UNPACK_BUFFER, 0, data.getSize(), data.getData());
	}
	...
	{
		_bufferSubData(_tlsArrayBuffer(), GL_PIXEL_UNPACK_BUFFER, offset, size, data.getSubData((int)offset, (int)size));
	}
	...
	{
		_bufferSubData(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, offset, size, data);
	}
	...
	{
		return (BufferAccess)_getBufferParameter(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, GL_BUFFER_ACCESS);
	}
	...
	{
		return _getBufferParameter(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, GL_BUFFER_MAPPED);
	}
	...
	{
		return _getBufferPointer(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER);
	}

	/*(2.1) Gets the size of the buffer object parameter value, measured in bytes. The initial value is 0
	@return The size of the buffer object*/
	GLint getBufferSize()
	{
		return _getBufferParameter(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, GL_BUFFER_SIZE);
	}
	...
	{
		_getBufferSubData(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, offset, size, data.getSubData((int)offset, (int)size));
	}
	...
	{
		_getBufferSubData(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, offset, size, data);
	}
	...
	{
		return (BufferUsage)_getBufferParameter(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, GL_BUFFER_USAGE);
	}
	...
	{
		_buffer_unbind(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER);
	}
	...
	{
		return _unmapBuffer(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER);
	}
	...
	{
		_flushMappedBufferRange(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, offset, length);
	}
	...
	{
		return (BufferFlags)_getBufferParameter(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, GL_BUFFER_ACCESS_FLAGS);
	}
	...
	{
		return _mapBufferRange(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, offset, length, (GLbitfield)access);
	}
	...
	{
		_buffer_copy(_tlsPixelUnpackBuffer(), _tlsCopyReadBuffer(), GL_COPY_READ_BUFFER);
	}

	/*(3.1) Explicitly binds buffer object to copy-write target, the previous binding is lost*/
	void bindCopyWriteBuffer()
	{
		_buffer_copy(_tlsPixelUnpackBuffer(), _tlsCopyWriteBuffer(), GL_COPY_WRITE_BUFFER);
	}
	...
	{
		_copyBufferSubData(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, _tlsArrayBuffer(), GL_ARRAY_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, _tlsElementArrayBuffer(), GL_ELEMENT_ARRAY_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, _tlsPixelPackBuffer(), GL_PIXEL_PACK_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}

	/*(3.1) (4) Copies all or part of the data store of a buffer object into the data store of the PixelUnpackBuffer object
	@param Specifies the destination buffer object. If unbound from its target, the object is being bind to copy-write target
	@param Specifies the offset, in basic machine units, within the data store of the source buffer object from which data will be read
	@param Specifies the offset, in basic machine units, within the data store of the destination buffer object into which data will be written
	@param Specifies the size, in basic machine units, of the data to be copied from the source buffer object to the destination buffer object*/
	...
	{
		GLuint* tls = _tlsPixelUnpackBuffer();
		_copyBufferSubData(tls, GL_PIXEL_UNPACK_BUFFER, tls, GL_PIXEL_UNPACK_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, _tlsTransformFeedbackBuffer(), GL_TRANSFORM_FEEDBACK_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, _tlsTextureBuffer(), GL_TEXTURE_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyBufferSubData(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, _tlsUniformBuffer(), GL_UNIFORM_BUFFER, (_Buffer&)writeBuffer, readOffset, writeOffset, size);
	}
	...
	{
		_copyWriteBufferData(_tlsPixelUnpackBuffer(), (GLsizeiptr)dataStore.getSize(), dataStore.getData(0), (GLenum)usage);
	}
	...
	{
		_copyWriteBufferData(_tlsPixelUnpackBuffer(), size, data, (GLenum)usage);
	}
	...
	{
		return (GLsizeiptr)_getBufferParameter64(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, GL_BUFFER_MAP_LENGTH);
	}
	...
	{
		return (GLintptr)_getBufferParameter64(_tlsPixelUnpackBuffer(), GL_PIXEL_UNPACK_BUFFER, GL_BUFFER_MAP_OFFSET);
	}
