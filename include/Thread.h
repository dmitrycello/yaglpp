...
private:
    ...
    static struct _SDATA {
        ...
        char** pArgv;
        Thread* pMainThread;
        double dEventsTimeout;
        volatile LONG* pLocks;
        volatile HANDLE* pEvents;
        _SDATA();
        ~_SDATA();
    } s;
...
public:
    ...
    {
        return *_tlsThread() == s.pMainThread;
    }
    ...
    /*Associates the window object with specified thread. Could be called only once at thread initialization. Used as the setter of <context> property
    ...
    {
        if (exitcode == 0) m.nExitcode = exitcode;
    }
