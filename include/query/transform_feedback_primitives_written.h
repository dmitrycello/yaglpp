...
#ifndef YAGLPP_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN_H
#define YAGLPP_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN_H

#ifdef YAGLPP_VERSION_3_0
namespace gl {
/*(3.0) OpenGL written primitives transform feedback query object class derived from <Queries>*/
class TransformFeedbackPrimitivesWritten : public Queries
{
public:
	...
	void Begin()
	{
		object_bind(glBeginQuery, GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN);
	}

	/*(3.0) Delimits the ending boundary of a query object. The query object must be active*/
	static void End()
	{
		end_query(GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN);
	}
	...
	{
		get_query(GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN, GL_QUERY_COUNTER_BITS);
	}
	...
	{
		return object_id() == get_query(GL_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN, GL_CURRENT_QUERY);
	}
...
#endif // #ifndef YAGLPP_TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN_H
