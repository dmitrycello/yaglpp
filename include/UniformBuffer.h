...
public:
	...
	{
		_bufferData(_tlsUniformBuffer(), GL_UNIFORM_BUFFER, (GLsizeiptr)dataStore.getSize(), dataStore.getData(), (GLenum)usage);
	}
	...
	{
		_bufferSubData(_tlsArrayBuffer(), GL_UNIFORM_BUFFER, 0, data.getSize(), data.getData());
	}
	...
	{
		_bufferSubData(_tlsArrayBuffer(), GL_UNIFORM_BUFFER, offset, size, data.getSubData((int)offset, (int)size));
	}
	...
	/*(3.1) Gets the access policy flag parameter value set while mapping the buffer object (the value of the access parameter bitfield passed to <mapBufferRange>). If the buffer was mapped with <mapBuffer>, the access policy is determined by translating the enums in that access parameter to the corresponding bits for <mapBufferRange> as described in the OpenGL Specification. Used as a getter of <bufferAccessFlags> property
	...
	/*(3.1) Gets the size of the buffer object parameter value, measured in bytes. Used as a getter of <bufferSize> property
	...
	{
		return _object_id() == _getInteger(GL_UNIFORM_BUFFER_BINDING);
	}
