...
{
	GLPP_ASSERT(m_pMonitor == nullptr); // GLFW MONITOR OBJECT MUST BE DESELECTED
	Monitor* pClass = (Monitor*)glfwGetMonitorUserPointer(monitor);
	if (pClass)
	{
		pClass->deselectMonitor();
...
{
	GLPP_ASSERT((int)vindex < m_iVidcount); // INVALID VIDEOMODE INDEX PARAMETER
...
GLFWmonitor* Monitor::_monitor()
{
	...
	{
		selectMonitor();
		GLPP_ASSERT(isMonitor()); // NO MONITORS IN THE SYSTEM WERE FOUND
	}
	return m_pMonitor;
}
