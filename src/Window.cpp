...
void Window::makeContextCurrent(_In_ Thread* thread)
{
	...
	YAGLPP_ASSERT(!isContextCurrent()); // GLFW WINDOW OBJECT HAS CURRENT CONTEXT
	YAGLPP_ASSERT(thread->isCurrentThread()); // METHOD IS CALLED BY ANOTHER THREAD
	glfwMakeContextCurrent(_window());
	if (sLoaded == 0)
	{
		sLoaded = _loadGL(glfwGetProcAddress);
		YAGLPP_ASSERT(sLoaded != 0); // FAILED TO LOAD OPENGL CONTEXT
	}
	m_pThread = thread;
}
...
{
	YAGLPP_ASSERT(getContextThread()->isCurrentThread()); // METHOD IS CALLED BY ANOTHER THREAD
...
{
	YAGLPP_ASSERT(m_pThread != nullptr); // GLFW WINDOW OBJECT HAS NO CURRENT CONTEXT
	return m_pThread;
}
#endif // #ifdef _DEBUG
} // namespace glfw
