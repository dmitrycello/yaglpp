...
{
#ifndef YAGLPP_COCOA_CHDIR_RESOURCES
...
void Window::_bindCallback()
{
	...
	m.iCursorEntered = GL_FALSE;
	glfwGetCursorPos(m.pWindow, &dCursorX, &dCursorY);
	if ((dCursorX >= 0.0) || (dCursorY >= 0.0))
	{
		int iWidth, iHeight;
		glfwGetWindowSize(m.pWindow, &iWidth, &iHeight);
		if ((dCursorX < iWidth) || (dCursorY < iHeight))
		{
			m.iCursorEntered = GL_TRUE;
		}
	}
	m.iWindowFocused = glfwGetWindowAttrib(m.pWindow, GLFW_FOCUSED);
	m.iWindowMaximized = glfwGetWindowAttrib(m.pWindow, GLFW_MAXIMIZED);
	m.iWindowVisible = glfwGetWindowAttrib(m.pWindow, GLFW_VISIBLE);
	getCursorPos();
	getFramebufferSize();
	...
	getWindowPos();
	getWindowSize();
	glfwSetCharCallback(m.pWindow, &_charCallback);
	glfwSetCursorEnterCallback(m.pWindow, &_cursorEnterCallback);
	glfwSetCursorPosCallback(m.pWindow, &_cursorPosCallback);
	glfwSetDropCallback(m.pWindow, &_dropCallback);
	glfwSetFramebufferSizeCallback(m.pWindow, &_framebufferSizeCallback);
	glfwSetKeyCallback(m.pWindow, &_keyCallback);
	glfwSetMouseButtonCallback(m.pWindow, &_mouseButtonCallback);
	glfwSetScrollCallback(m.pWindow, &_scrollCallback);
	glfwSetWindowCloseCallback(m.pWindow, &_windowCloseCallback);
	glfwSetWindowContentScaleCallback(m.pWindow, &_windowContentScaleCallback);
	glfwSetWindowFocusCallback(m.pWindow, &_windowFocusCallback);
	glfwSetWindowIconifyCallback(m.pWindow, &_windowIconifyCallback);
	glfwSetWindowMaximizeCallback(m.pWindow, &_windowMaximizeCallback);
	glfwSetWindowPosCallback(m.pWindow, &_windowPosCallback);
	glfwSetWindowRefreshCallback(m.pWindow, &_windowRefreshCallback);
	glfwSetWindowSizeCallback(m.pWindow, &_windowSizeCallback);
	glfwSetWindowUserPointer(m.pWindow, this);
}

void Window::_charCallback(GLFWwindow* window, unsigned int codepoint)
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	pWindow->m.uCharCodepoint = codepoint;
	pWindow->onChar(codepoint);
	Thread* pThread = pWindow->m.pThread;
	if ((pThread != nullptr) && (pThread != Thread::s.pMainThread))
	{
...
void Window::_create(int width, int height, const char* title, GLFWwindow* shared, ContextSetting* setting)
{
	...
	m.pWindow = glfwCreateWindow(width, height, title, nullptr, shared);
...
void Window::_createFullscreen(const char* title, Monitor& monitor, GLFWwindow* shared, ContextSetting* setting)
{
	...
	m.pWindow = glfwCreateWindow(pVidmode->width, pVidmode->height, title, monitor._monitor(), shared);
	_YAGLPP_ASSERT_(isWindow()); // FAILED TO CREATE GLFW WINDOW
	_bindCallback();
}
...
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	pWindow->m.iCursorEntered = entered;
...
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	pWindow->m.dCursorPosX = xpos;
	pWindow->m.dCursorPosY = ypos;
...
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	pWindow->m.iFramebufferWidth = width;
	pWindow->m.iFramebufferHeight = height;
...
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	pWindow->m.iKeyboardKey = key;
	pWindow->m.iKeyScancode = scancode;
	pWindow->m.iKeyAction = action;
	pWindow->m.iKeyMods = mods;
	if ((pWindow->m.iWindowClose == GLFW_TRUE) && (action == GLFW_RELEASE) && (mods == 0))
	{
...
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	pWindow->m.iMouseButton = button;
	pWindow->m.iMouseAction = action;
	pWindow->m.iKeyMods = mods;
...
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	pWindow->m.dScrollOffsetX = xoffset;
	pWindow->m.dScrollOffsetY = yoffset;
...
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	if (pWindow->m.iKeyMods != 0)
	{
		pWindow->m.iWindowClose = GLFW_TRUE; // Wait for exit flag
...
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	pWindow->m.fWindowContentScaleX = xscale;
	pWindow->m.fWindowContentScaleY = yscale;
...
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	pWindow->m.iWindowFocused = focused;
...
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	pWindow->m.iWindowIconified = iconified;
...
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	pWindow->m.iWindowMaximized = maximized;
...
{
	Window* pWindow = (Window*)glfwGetWindowUserPointer(window);
	if (pWindow->m.iWindowFullscreen == GLFW_FALSE)
	{
		pWindow->m.iWindowPosX = xpos;
		pWindow->m.iWindowPosY = ypos;
...
void Window::_windowSizeCallback(GLFWwindow* window, int width, int height)
{
	...
	if (pWindow->m.iWindowFullscreen == GLFW_FALSE)
	{
		pWindow->m.iWindowWidth = width;
		pWindow->m.iWindowHeight = height;
...
void Window::destroyWindow()
{
	...
	else Thread::s.pMainThread->_message(YAGLPP_DESTROY_WINDOW, 0, 0, 0, 0, (LPVOID)this);
}
...
void Window::focusWindow()
{
	if (isMainThreadCurrent())
	{
		...
		m.iWindowFocused = GLFW_TRUE;
	}
	else Thread::s.pMainThread->_message(YAGLPP_FOCUS_WINDOW, 0, 0, 0, 0, (LPVOID)this);
}
...
void Window::hideWindow()
{
	if (isMainThreadCurrent())
	{
		...
		m.iWindowVisible = GLFW_FALSE;
	}
	else Thread::s.pMainThread->_message(YAGLPP_HIDE_WINDOW, 0, 0, 0, 0, (LPVOID)this);
}
...
void Window::iconifyWindow()
{
	if (isMainThreadCurrent())
	{
		...
		m.iWindowIconified = GLFW_TRUE;
	}
	else Thread::s.pMainThread->_message(YAGLPP_ICONIFY_WINDOW, 0, 0, 0, 0, (LPVOID)this);
}

void Window::makeContextCurrent()
{
	static int sLoaded = 0;
	_YAGLPP_ASSERT_(m.pThread == nullptr); // WINDOW'S CONTEXT HAS BEEN MADE CURRENT
	Thread* pThread = getCurrentThread();
	if (pThread->m.pWindow)
	{
		pThread->m.pWindow->m.pThread = nullptr;
	}
	m.pThread = pThread;
	pThread->m.pWindow = this;
	glfwMakeContextCurrent(_window());
	if (sLoaded == 0)
	{
		sLoaded = _loadGL(glfwGetProcAddress);
		_YAGLPP_ASSERT_(sLoaded != 0); // FAILED TO LOAD OPENGL CONTEXT
	}
}
...
{
	Thread* pThread = getCurrentThread();
	_YAGLPP_ASSERT_(pThread == m.pThread); // WINDOW'S CONTEXT HAS NOT BEEN MADE CURRENT
	m.pThread = nullptr;
...
void Window::maximizeWindow()
{
	if (isMainThreadCurrent())
	{
		...
		m.iWindowMaximized = GLFW_TRUE;
	}
	else Thread::s.pMainThread->_message(YAGLPP_MAXIMIZE_WINDOW, 0, 0, 0, 0, (LPVOID)this);
}
...
void Window::requestWindowAttention()
{
	...
	else Thread::s.pMainThread->_message(YAGLPP_REQUEST_WINDOW_ATTENTION, 0, 0, 0, 0, (LPVOID)this);
}
...
void Window::restoreWindow()
{
	if (isMainThreadCurrent())
	{
		...
		m.iWindowIconified = GLFW_FALSE;
		m.iWindowMaximized = GLFW_FALSE;
	}
	else Thread::s.pMainThread->_message(YAGLPP_RESTORE_WINDOW, 0, 0, 0, 0, (LPVOID)this);
}
...
void Window::setClipboardString(_In_z_ const char* string)
{
	...
	else Thread::s.pMainThread->_message(YAGLPP_SET_CLIPBOARD_STRING, _copyString(string), 0, 0, 0, (LPVOID)this);
}
...
void Window::setCursorPos(double xpos, double ypos)
{
	...
	else Thread::s.pMainThread->_message(YAGLPP_SET_CURSOR_POS, xpos, ypos, 0, 0, (LPVOID)this);
}
...
void Window::setWindowAspectRatio(int numer, int denom)
{
	...
	else Thread::s.pMainThread->_message(YAGLPP_SET_WINDOW_ASPECT_RATIO, numer, denom, 0, 0, (LPVOID)this);
}
...
void Window::setWindowMonitor()
{
	...
	else Thread::s.pMainThread->_message(YAGLPP_SET_WINDOW_MONITOR, 0, 0, 0, 0, (LPVOID)this);
}
...
void Window::setWindowOpacity(float opacity)
{
	...
	else Thread::s.pMainThread->_message(YAGLPP_SET_WINDOW_OPACITY, opacity, 0, 0, 0, (LPVOID)this);
}
...
void Window::setWindowPos(int xpos, int ypos)
{
	if (isMainThreadCurrent())
	{
		_YAGLPP_ASSERT_(m.iWindowFullscreen == GLFW_FALSE); // MUST NOT BE USED IN FULLSCREEN MODE
		glfwSetWindowPos(_window(), xpos, ypos);
		m.iWindowPosX = xpos;
		m.iWindowPosY = ypos;
	}
	else Thread::s.pMainThread->_message(YAGLPP_SET_WINDOW_POS, xpos, ypos, 0, 0, (LPVOID)this);
}
...
void Window::setWindowSize(int width, int height)
{
	if (isMainThreadCurrent())
	{
		...
		m.iWindowWidth = width;
		m.iWindowHeight = height;
	}
	else Thread::s.pMainThread->_message(YAGLPP_SET_WINDOW_SIZE, width, height, 0, 0, (LPVOID)this);
}
...
void Window::setWindowSizeLimits(int minwidth, int minheight, int maxwidth, int maxheight)
{
	...
	else Thread::s.pMainThread->_message(YAGLPP_SET_WINDOW_SIZE_LIMITS, minwidth, minheight, maxwidth, maxheight, (LPVOID)this);
}
...
void Window::setWindowTitle(_In_z_ const char* title)
{
	...
	else Thread::s.pMainThread->_message(YAGLPP_SET_WINDOW_TITLE, _copyString(title), 0, 0, 0, (LPVOID)this);
}
...
void Window::showWindow()
{
	if (isMainThreadCurrent())
	{
		...
		m.iWindowVisible = GLFW_TRUE;
	}
	else Thread::s.pMainThread->_message(YAGLPP_SHOW_WINDOW, 0, 0, 0, 0, (LPVOID)this);
}
...
void Window::unsetCursor()
{
	...
	else Thread::s.pMainThread->_message(YAGLPP_UNSET_CURSOR, 0, 0, 0, 0, (LPVOID)this);
}
...
void Window::unsetWindowAspectRatio()
{
	...
	else Thread::s.pMainThread->_message(YAGLPP_UNSET_WINDOW_ASPECT_RATIO, 0, 0, 0, 0, (LPVOID)this);
}
...
void Window::unsetWindowIcon()
{
	...
	else Thread::s.pMainThread->_message(YAGLPP_UNSET_WINDOW_ICON, 0, 0, 0, 0, (LPVOID)this);
}
...
void Window::unsetWindowMonitor()
{
	if (isMainThreadCurrent())
	{
		glfwSetWindowMonitor(_window(), nullptr, m.iWindowPosX, m.iWindowPosY, m.iWindowWidth, m.iWindowHeight, GLFW_DONT_CARE);
		m.iWindowFullscreen = GLFW_FALSE;
	}
	else Thread::s.pMainThread->_message(YAGLPP_UNSET_WINDOW_MONITOR, 0, 0, 0, 0, (LPVOID)this);
}
...
{
	_YAGLPP_ASSERT_(isMainThreadCurrent()); // MUST BE CALLED ONLY FROM THE MAIN THREAD
	glfwGetCursorPos(_window(), &m.dCursorPosX, &m.dCursorPosY);
}

void Window::getFramebufferSize()
{
	_YAGLPP_ASSERT_(isMainThreadCurrent()); // MUST BE CALLED ONLY FROM THE MAIN THREAD
	glfwGetFramebufferSize(_window(), &m.iFramebufferWidth, &m.iFramebufferHeight);
}
...
{
	_YAGLPP_ASSERT_(isMainThreadCurrent()); // MUST BE CALLED ONLY FROM THE MAIN THREAD
	glfwGetWindowContentScale(_window(), &m.fWindowContentScaleX, &m.fWindowContentScaleY);
}

void Window::getWindowFrameSize()
{
	_YAGLPP_ASSERT_(isMainThreadCurrent()); // MUST BE CALLED ONLY FROM THE MAIN THREAD
	glfwGetWindowFrameSize(_window(), &m.iWindowFrameLeft, &m.iWindowFrameTop, &m.iWindowFrameRight, &m.iWindowFrameBottom);
}
...
void Window::getWindowPos()
{
	_YAGLPP_ASSERT_(isMainThreadCurrent()); // MUST BE CALLED ONLY FROM THE MAIN THREAD
	_YAGLPP_ASSERT_(m.iWindowFullscreen == GLFW_FALSE); // MUST NOT BE USED IN FULLSCREEN MODE
	glfwGetWindowPos(_window(), &m.iWindowPosX, &m.iWindowPosY);
}
...
void Window::getWindowSize()
{
	_YAGLPP_ASSERT_(isMainThreadCurrent()); // MUST BE CALLED ONLY FROM THE MAIN THREAD
	_YAGLPP_ASSERT_(m.iWindowFullscreen == GLFW_FALSE); // MUST NOT BE USED IN FULLSCREEN MODE
	glfwGetWindowSize(_window(), &m.iWindowWidth, &m.iWindowHeight);
}
