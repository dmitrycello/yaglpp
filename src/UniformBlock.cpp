...
{
	_YAGLPP_ASSERT_(_mpData == nullptr); // OPENGL UNIFORM BLOCK MUST BE EMPTY
	...
	_mpData = (_LPDATA)_YAGLPP_MALLOC_(sizeof(_DATA) + sizeof(GLuint) * iCount);
	_mpData->index = index;
...
{
	_YAGLPP_ASSERT_(_mpData != nullptr); // OPENGL UNIFORM BLOCK OBJECT IS EMPTY
	return _mpData;
}
