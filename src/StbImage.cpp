...
#define YAGLPP_STBIMAGE_ASSERT if (m_lpPixels == nullptr) { std::cout << "STB IMAGE ERROR: " << stbi_failure_reason() << std::endl; YAGLPP_ASSERT(false); }
#else // #ifdef _DEBUG
...
{
	YAGLPP_ASSERT(m_lpPixels == nullptr); // STB IMAGE DESTINATION OBJECT HAS LOADED IMAGE
	...
	m_lpPixels = YAGLPP_MALLOC(iSize);
	memcpy(m_lpPixels, lpSource, iSize);
}

void StbImage::copyRegion(const StbImage& source, int rleft, int rtop, int rwidth, int rheight)
{
	YAGLPP_ASSERT((this == &source) || (m_lpPixels == nullptr)); // STB DESTINATION IMAGE OBJECT HAS LOADED IMAGE
	void* lpSource = source.getPixels(); // Pixels
	m_iByte = source.m_iByte;
	m_iComp = source.m_iComp;
	YAGLPP_ASSERT((rwidth > 0) && (rleft >= 0) && (source.m_iWidth >= (rleft + rwidth))); // INVALID WIDTH OR LEFT PARAMETERS OF ALLOCATING SPRITE
	YAGLPP_ASSERT((rheight > 0) && (rtop >= 0) && (source.m_iHeight >= (rtop + rheight))); // INVALID HEIGHT OR TOP PARAMETERS OF ALLOCATING SPRITE
	int iDstPos = 0,
		iPixel = m_iByte * m_iComp,
		iDstLine = rwidth * iPixel,
		iSrcLine = source.m_iWidth * iPixel,
		iSrcPos = (iSrcLine * rtop) + (rleft * iPixel);
	m_lpPixels = YAGLPP_MALLOC(static_cast<size_t>(iDstLine) * rheight);
...
{
	YAGLPP_ASSERT(m_lpPixels == nullptr); // STB IMAGE OBJECT HAS LOADED IMAGE
	...
	YAGLPP_ASSERT((m_iByte > 0) && (m_iComp > 0)); // IMAGE PIXEL FORMAT MUST BE NON-DEFAULT
	int iSize = m_iWidth * m_iHeight * m_iByte * m_iComp;
	m_lpPixels = (init) ? YAGLPP_CALLOC(iSize) : YAGLPP_MALLOC(iSize);
}
...
bool StbImage::resizeRegion(const StbImage& source, int width, int height, int rleft, int rtop, int rwidth, int rheight)
{
	...
	STBIR_RESIZE resize;
	m_lpPixels = YAGLPP_MALLOC(static_cast<size_t>(width) * height * m_iByte * m_iComp);
...
{
	YAGLPP_ASSERT(m_iByte == 1); // STB IMAGE DATA MUST BE 8-BITS PER CHANNEL
...
{
	YAGLPP_ASSERT(m_iByte == 4); // STB IMAGE DATA MUST BE 32F-BITS PER CHANNEL
...
{
	YAGLPP_ASSERT(m_lpPixels != nullptr); // STB IMAGE OBJECT HAS NO LOADED IMAGE
	return m_lpPixels;
}
#endif // #ifdef _DEBUG
