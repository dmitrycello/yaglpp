...
### GLAD class tree
Most of the classes in **`::gl`** namespace are derived from **`gl::_Object`**, operating a single _id_. The data members of **`Uniform`** and **`VertexAttrib`** classes are single signed integers, operating a location value. It could be **`-1`**, meaning an empty object, or invalid index. The classes derived from **`gl::_Objects`** are multi-objects, they are dynamically created blocks of client memory carriyng an array of OpenGL object _ids_ of varriyng length. The class data member for classes derived from **`gl::_Objects`**, **`Sync`** and **`UniformBlock`**, is a pointer. Its value **`NULL`** means an empty object. The following class tree exposes the classes inheritance, as well as their data size in bytes:
