...
### GLAD class tree
Most of the classes in **`::gl`** namespace are derived from **`gl::_Object`**, operating a single _id_. The data member of **`Uniform`** and **`VertexAttrib`** classes is a single signed integer, operating a location value. It could be **`-1`**, meaning an invalid index, or an empty object. The classes derived from **`gl::_Objects`** are multi-objects, carriyng an array of OpenGL object _ids_ of varriyng length. Their class data member is single pointer, as well as for **`Sync`** and **`UniformBlock`** classes. Its value **`NULL`** means an empty object. The following class tree exposes the classes inheritance, as well as their data size in bytes:
